...
;; @(#) $Id: jsee.el,v 1.1 2005-11-26 01:15:01 hute37 Exp $
...
(defcustom jde-jsee-doc-generator "polardoc"
...
(defcustom jde-jsee-javadoc--d-directory "$TEMP/"
  "*The working directory in which javadoc will generate HTML files.
This directory must exists. For better results you could copy in it
the `images' sub-directory of the JDK API documentation directory."
  :group 'jde-jsee
...
(defun jsee-get-package-name ()
  "Straight copied from `jde-wiz-get-package-name'.  Updated by Steve to
 only match a package if it is the first thing on a line, this means a // 
 will cancel the package."
...
(defun jsee-get-javadoc-options ()
  ...
  (let ((options (jsee-build-javadoc-classpath-option)))
    ...
    (if jde-jsee-javadoc-protected-option
        (setq options (concat options " -protected -private")))
