...
# @(#) $Id: makefile.pl,v 1.1 2005-12-04 20:58:09 hute37 Exp $
#
#  File id
#
...
#	.Created:	2000-01
#	.Keywords:	Perl, tiny tools, makefile
#	.Perl:	        5.004
#
...
#
#	If you see "#font" comments inside the code that are there so that
#	Emacs font-lock.el moduel will fontify with color the file
#	corrctly. The Perl programs are difficult to parse and sometimes
#	the font-lock extends a wrong color over multiple lines. The
#	#font tag and seom magic perl code markers will stop the wrong
#	fontification. To use font-lock in Emacs, use
#
#	    ;; $HOME/.emacs
#
#	    (require 'font-lock)                 ;; '
...
#   Scratchboard
#
#	perl ../bin/makefile.pl -v --html-regexp "README" --html-frame --Button-previous http://tiny-tools.sourceforge.net/ html
#	perl ../bin/makefile.pl -v --html-regexp "emacs-.*txt" --html-frame --Button-previous http://tiny-tools.sourceforge.net/ html#
#   End
...
sub Initialize ()
{
    ...
    use vars qw ($SF_INPUT_FTP_SITE $SF_INPUT_FTP_DIR);

    $SF_INPUT_FTP_SITE = "upload.sourceforge.net";
    $SF_INPUT_FTP_DIR  = "/incoming";
    ...
    $FILE_ID  = q$Id: makefile.pl,v 1.1 2005-12-04 20:58:09 hute37 Exp $; # m:
...
@(#) makefile.pl - Perl Tiny Tools kit makefile
...
Compile to using "emacs" or "xemacs". The program binary must be
given.

=item B<--Bzip>

Compress all files using bzip2(1)

=item B<--Gzip>

Compress all files using gzip(1)

=item B<--html-base BASE-URL>

Specify location where html page will be kept.
all HTML files will have HTML tag BASENAME with value BASE-URL. All
#URL-BASE link references in the text also reflect this location.
See t2html.pl's B<--base> option.

=item B<--html-frame>

Allow generating framed html pages when making target "html".
You have to use B<--html-base> with this option.


=item B<--html-regexp REGEXP>

Generate HTML only form files matching regexp.
...
=item B<--perlfix INTERPRETER>

The normal perl location is /usr/local/bin/perl which is carved
to all of the perl programs. some systems (e.g. default Linux RedHat)
may have perl in another location, perhaps /usr/bin/perl

This command will substitute all .pl files to contain INTERPRETER
absolute path in "#!" line.
...
Build a TARGET package which can be C<deb>  C<rpm> C<tgz> C<zip>

=item B<--Link-check REGEXP>

Validate all URL links for all files in doc/*.txt

=item B<sf-html>

Generate HTML pages by using sourceforge as destination.

=item B<sf-upload FILE>

Upload the currnt package release FILE to sourceforge FTP site, from
where it can be transferred via "Project Admin" page to file releases.
See http://www.sourceforge.net/ => (Left Hand panel)Site Docs/
Project Administration/Making File Releases

=item B<autoload>

Generate autoload file tiny-autoload.el

=item B<--Tempdir DIR>

Use this directory to build. This dfeaults to environemtn variable
$TEMPDIR, $TEMP, /tmp, ~/tmp or whatever directory is found appropriate.
...
sub HandleCommandLineArgs ()
{
    ...
    if ( defined $GZIP  and  defined $BZIP )
    {
	die "$id: CAn't use both --gzip and --bzip options.";
    }
    ...
    {
	unless ( -f $PERL_PATH  or -f "${PERL_PATH}.exe" )
	{
	    die "$id: --perlfix PATH option, path not found: [$PERL_PATH]";
	}
    }

    $upload	and  SfUploadIncoming( $upload );
}
...
#   DESCRIPTION
#
#       Fix all file's #! to contain PATH
#
#   INPUT PARAMETERS
#
#	$file	    File name,
...
sub FixFileShebang ( $ $ )
{
    ...
    {
	warn "$id: cannot open for read [$file] $ERRNO";
	return;
    }
    ...
    for ( @content )
    {
	if ( /^#!(.*)/ )		    # must find absolute path c:/temp
	{
	    my $old = $1;
	...
	unless ( $test )
	{
	    unless ( open FILE, "> $file" )
	    {
		warn "$id: cannot open for write [$file] $ERRNO";
		return;
	    }
	...
	if ( $debug )
	{
	    print "$id: Wrote $file with $path\n"
		, @content[0..2], "\n";
	}
    }
    else
    {
	$debug  and  print "$id: File ok $file with $path\n";
    }
...
#   DESCRIPTION
#
#       Fix all perl files to contain correct perlinterpreter
#
#   INPUT PARAMETERS
#
#	$dir	    Directory where to look
#	$path	    Path for #!
#
#   RETURN VALUES
#
...
sub TargetPerlfix ( $ $ )
{
    ...
    $dir = PathConvert $dir, -unix;

    $debug  and  print "$id: reading dir $dir\n";


    unless ( opendir DIR, $dir )
    {
	warn "$id: cannot open [$dir] $ERRNO";
	return;
    }

    while ( defined($ARG = readdir DIR) )
    {
	my $p = "$dir/$ARG";

	if ( -f $p and  /\.pl$/ )
	{
	    FixFileShebang $p, $path;
	}
    }
...
#   DESCRIPTION
#
#       Create RPM Build directories under DIR
#
#   INPUT PARAMETERS
#
#	$dir	Supposed to be ~/tmp
#
#   RETURN VALUES
#
#	none
...
sub RpmMakeBuildDirs ($)
{
    my $id = "$LIB.RpmMakeBuildDirs";
    my ($dir) = @ARG;

    not -d $dir	 and  die "$id:  You must create directory $dir first.\n";
    ...
    for ( @rpmDirs )
    {
	my $path = "$dir/redhat/$ARG";
	unless ( -d $path )
	{
	    $debug  and  print "$id: Making dir $path\n";

	    unless ( $test )
	    {
		mkpath $path  or   die "$id: Cannot create: $path";
	    }
	}
    }
}
...
sub TargetBuildRpm ()
{
    my $id    = "$LIB.TargetBuildRpm";

    my $tmp   = TempDir();
    my $bin   = "rpm";
    my ($RPM) = FindBinary $bin, "." ;

    not $RPM	    and  die "$id: can't find binary: $bin in PATH.";

    $verb  and  print "$id: started\n";

    RpmMakeBuildDirs $tmp ;
...
#   DESCRIPTION
#
#       Run the build options.
#
#   INPUT PARAMETERS
#
#	@	List of build targets: rpm, deb, tgz, zip
#
#   RETURN VALUES
#
#	none
...
sub TargetBuild ( @ )
{
	...
	{
	    warn "$id: No build option $function implemented yet.";
	}
    }
}
...
$str: About to run external shell command
      ...
      contact maintainer $CONTACT immediately.
EOF
}
...
#   DESCRIPTION
#
#	Validate URL links in DIR matching REGEXP.
#
#   INPUT PARAMETERS
#
#	$dir	    From this directory
#	$regexp	    Files matching regexp
#
#   RETURN VALUES
#
#	none
...
sub LinkCheck ( $$ )
{
    ...
    unless ( opendir DIR, $dir )
    {
	warn "$id: ERROR on dir [$dir] $ERRNO";
	return;
    }
	...
	if ( /$regexp/ )
	{
	    my $cmd = "perl -S t2html.pl --link-check-single";

	    $verb   and  $cmd .= " -d --verbose";
	    $cmd    .= " $dir/$ARG";
...
sub CheckNeededDirs (@)
{
	...
	print<<"EOF";
$id:
    Your directories are not what is expected. This may be due to
    ...
    d)  The directory structure has changed as this file was not updated
...
#   DESCRIPTION
#
#	Upload release package to sourceforge
...
sub SfUploadIncoming ($)
{
    my $id     = "$LIB.SfUploadIncoming";
    my ($file) = @ARG;

    eval "use Net::FTP";      # Load package at run-time
    $debug and warn "$id: eval [$EVAL_ERROR] \n";

    return if $EVAL_ERROR;

    my $host = $SF_INPUT_FTP_SITE;
    my $dir  = $SF_INPUT_FTP_DIR;

    my $ftp;

    unless ( $ftp = Net::FTP->new( $host , Debug => $debug) )
    {
	die "$id: Can't extablish FTP->new socket.";
    }


    $ftp->login( "anonymous", 'tiny-tools@example.com' )
	or die "$id: login error $ERRNO, server full? Use --debug";

    $ftp->cwd($dir)  or die "$id: cwd $dir error $ERRNO";
    $ftp->binary()   or die "$id: binary mode error $ERRNO";
    $ftp->put($file) or die "$id: put $file error $ERRNO"; ;
    $ftp->quit;
}
...
sub Main ()
{
    ...
    @BUILD	    and  TargetBuild @BUILD;
    $LINK_CHECK	    and	 LinkCheck( $DOC, '\.txt$' );
    ...
    if ( defined $PERL_PATH )		# if --perlfix given, add target
    {
	push @ARGV, "perlfix";
    }
...
EOF
	...
	SWITCH:
	{
	    ...
	    /^perlfix/	    and TargetPerlfix($BIN, $PERL_PATH), last;
    ...
    {
	$verb  and  print "$id: done. makefile.pl $VERSION\n";
    }
