...
;; $Id: dired-sort.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
;; This file is not part of Emacs

;; {{{ Id

;; Copyright (C)    1989 "fad" -- whoever it was
;; Author:          unknown
;; Maintainer:	    <jari.aalto@poboxes.com>
...
;;
;;	    s" "    dired-sort-resort    (that's an "s" and SPACE)
...
(eval-and-compile
  ...
  (defvar revert-buffer-function) ;; FIXME: What is this?
  (defvar current-prefix-arg)     ;; Elint.el pedantic check silencer
...
(defun dired-sort-default-keys ()
  ...
  (let* ((map (symbol-value 'dired-mode-map)))
    ...
    (define-key map  "s " 'dired-sort-resort)
    (define-key map  "ss" 'dired-sort-by-size)
    (define-key map  "sd" 'dired-sort-by-date)
    (define-key map  "sf" 'dired-sort-by-field)
    (define-key map  "sn" 'dired-sort-by-name)
    (define-key map  "st" 'dired-sort-by-type)))
