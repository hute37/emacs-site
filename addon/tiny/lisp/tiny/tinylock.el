...
;;; @(#) $Id: tinylock.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    extensions
;;
;; To get information on this program use ident(1) or do M-x tinylock-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinylock|Jari Aalto|jari.aalto@poboxes.com|
;; Simple emacs locking utility|
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinylock.el.Z|
...
Input:
  ...
  (ti::xe-timer-cancel-function 'tinylock-process)
...
If LOCK-NOW is non-nil emacs is immediately locked with PSW."
  ...
  (let* (
	...
	(run-hooks 'tinylock-:before-lock-hook))
      ...
      (ti::xe-timer-list-control     'kill)	;Stop all timers
