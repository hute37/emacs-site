...
;;; @(#) $Id: tinytag.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
;;; $PackageInstallRe: '^;;+[*]' $
...
;; Keywords:        tools
;;
;; To get information on this program use ident(1) or do M-x tinytag-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinytag|Jari Aalto|jari.aalto@poboxes.com|
;; Tags like dataabase search, show std C/C++ function syntax in echo line|
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinytag.el.Z|
...
(ti::macrof-debug-standard "tinytag" "-:")
  ...
  (let* ((hook  (if (boundp 'post-command-idle-hook)
    ...
    (cond
     ((ti::idle-timer-supported-p)
      (ti::xe-timer-cancel-function 'tinytag-post-command)
      (unless uninstall
        (setq
...
(defun tinytag-install-sample-database-java ()
  ...
  (interactive)
  (tinytag-initialize)
  (let* ((java-info  (ti::process-java-version))
	 (case-fold-search t))
    (cond
     ((not java-info)
      (message
...
References:
  ...
  (let* ((tinytag-:noerror  t)
         it-is-time)
    (when (and (not (ti::xe-executing-macro))
