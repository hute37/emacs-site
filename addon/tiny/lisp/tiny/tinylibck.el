...
;;; @(#) $Id: tinylibck.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Created:      1996-03
;;
;; To get information on this program use ident(1) or do M-x ti::ck-version
...
(defun ti::ck-get-key-code (simple-key-sequence)
  ...
  (let* (lisp-mode-hook                 ;don't run any hooks while here
    ...
    ;; XEmacs doens't have this variable, Quiet ByteCompiler warning.
    ;; This is no-op
...
;;;	  (ti::d! elt str gather-flag gather-str list)
	  (cond
	   (elt
	    (setq elt (intern elt))
...
(defun ti::ck-xe2gnu-list (list)
  ...
  (let* ((table ti::ck-:key-table)
    ...
    (while list
      (setq elt (car list))
;;;      (ti::d! (car list) (rassq elt table) list)
