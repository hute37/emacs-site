...
;;; @(#) $Id: tinylibt.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
(ti::package-defgroup-tiny TinyMatch ti::text-: extensions
...
(defcustom ti::text-:face-table
  (list
   ...
   (cons 'warn   (if (emacs-p) 'region 'bold)) ;XE doesn't have 'region face
   (cons 'head    'bold))
...
`ti::text-:stack-push-flag' is set to 'undo-func while this function runs."
  ...
  (let* ((ti::text-:stack-push-flag  'undo-func)
    ...
    (if (not (and (ti::listp prev) (nth 0 prev)))
	(progn
	  (ti::text-stack-clear)
...
;;;	  (ti::d! rval list propl)
	  ...
	  (when delete
;;;	  (goto-char point)  (ti::d! point read-list)
...
;;;	(ti::d! (current-buffer) level (match-string count) (match-string 0) re)
...
Input:
 ...
 END		end point of search; defaults to `point-max'"
  (let* ((sprop (car match-plist))	;serach property
    ...
    (goto-char beg)  (setq point (1- (point)))
