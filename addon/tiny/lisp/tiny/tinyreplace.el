...
;;; @(#) $Id: tinyreplace.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    tools
;;
;; To get information on this program use ident(1) or do M-x tinyreplace-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyreplace|Jari Aalto|jari.aalto@poboxes.com|
;; 2002-08-02, hilit, block area support, latex, words, case pserve |
;; 28-Sep-1997|$Revision: 1.1 $|~/misc/tinyreplace.el.Z|
...
;;   ~/.emacs startup file
;; - Code can be easily extracted with tinylib.el / ti::package-rip-magic
;;
;;* _
;;*     (require 'tinyreplace)
;;
...
;;
;; For easy access to replace functions, use echo menu like this
;;* _
;;*     (ti::use-prefix-key global-map "\C-z") ;; make key available
;;* 	(global-set-key "\C-z%" 'tinyreplace-menu) ;; or C-z5 for non-shifted key
;;
;; Check that you have colors on, otherwise the replaced region may not be visible.
;;_
...
(defcustom tinyreplace-:user-function 'ti::buffer-xtra-open-outline
...
(defvar tinyreplace-:menu
   '("\
replace: (5)fwd (r)eg (w)ord (c)ompile buffer files (f)iles (lL)atex (?)help "
     (
...
5  calls function `tinyreplace-replace-forward'
%  calls function `tinyreplace-replace-forward'
...
This is done only if function exists. MODE can be 'write or 'read."
  (when (and (emacs-p)			;#todo: zmacs-region-stays
...
Note:
  ...
  (let* ((o-exclude  tinyreplace-:o-exclude)
	...
	(setq ans ?U))
       ...
       ((char-in-list-case ans '(?\177 ?\b ?n))
	(setq ans ?n)))

      (if (char-in-list-case ans '(?! ?? ?y ?n ?q ?Q ?b ?v ?u  ?B  ?N))
...
(defun tinyreplace-move-overlay (beg end)
  ...
  (ti::xe-overlay-move 'tinyreplace-:replace-region-overlay beg end  nil)
  (ti::xe-overlay-put
...
(defun tinyreplace-key-yank-word ()
  ...
  (let* (word)
    (with-current-buffer tinyreplace-:replace-buffer
      ...
      (if (char-in-list-case (preceding-char) '(?\ ?\t ?\n ))
	  (forward-char 1))
...
;;;	    (ti::d! "maxp" func (< (point) maxp) maxp (marker-position MARK-MAXP))
...
(defun tinyreplace-read-compile-buffer-filename ()
  ...
  (let ((re "^\\(\\(.:\\)?[^\n:]+\\):")   ;; allow DOS drive at front d:/file/
	...
	(if (and dir (not (string-match "^\\(.:\\)?/" file)))
...
;;;	(ti::d! (point) file (ti::read-current-line) )
		  ...
		  (if (and
			...
			(setq
			 ...
			 (ti::read-char-safe-until
			  (format
			   "\
%s: SPC s)ave, DEL n)ext, k)save and kill C)ontinue all Q)uit-exit"
			   (file-name-nondirectory file))))
			'(?s ?S  ?n ?N ?\b  ?\ ?C ?Q )))
		      (cond
		       ((char-in-list-case ch '(?\ ?s ?S))
			(with-current-buffer buffer (save-buffer)))

		       ((char-in-list-case ch '(?n ?N ?\b))
			nil)

		       ((char-in-list-case ch '(?Q))
			(throw 'exit t))

		       ((char-in-list-case ch '(?C))
			(with-current-buffer buffer (save-buffer))
			(setq no-confirm t))

		       ((char-in-list-case ch '(?k ?K))
...
(defun tinyreplace-replace-region (beg end str1 str2)
  ...
  (tinyreplace-replace-region-1 beg end (regexp-quote str1) str2 0 t))
...
you can use. Normally C - l yanks, and \"\\\" key deletes line."
  ...
  (tinyreplace-replace-region-1 (point) (point-max) (regexp-quote str1) str2 0 t))
