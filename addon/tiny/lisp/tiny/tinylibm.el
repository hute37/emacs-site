...
;;; @(#) $Id: tinylibm.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tinylibm-version
...
(progn  ;; eval-and-compile
  ...
  (let ((msg ""))
    ...
    (when (assoc "-debug-init" command-switch-alist)
      (message msg))
...
;;; ----------------------------------------------------------------------
;;; by Carl D. Roth" <roth@cse.ucsc.edu> in private mail exchange.
;;;
(defmacro-maybe definteractive (&rest body)
...
want to define simple key functions
 ...
 (global-set-key
   \"\\C-cc\"
   (definteractive (message \"You gave arg: %s\" (ti::prefix-arg-to-text arg))))"
...
If you just want to check, but not actually load package, use this
This macro also seacrhes [FEATURE|PACKAGE].el.gz files.

  (when-package 'browse-url nil
    ;;  I have set the binding, let Emacs know where to find the function.
    (autoload 'browse-url-at-mouse \"browse-url\" t t))"
  (`
   (when (or (and (, feature) (featurep (, feature)))
	     (locate-library (or (, package) (symbol-name (, feature))))
	     (locate-library
	      (concat (or (, package)
			  (symbol-name (, feature)))
		      ".el.gz")))
...
(defmacro-maybe int-to-float (nbr)
  "Tinylibm.el: Convert integer NBR to float."
...
;;; ----------------------------------------------------------------------
;;; - Can't remember where this originates, not mine...
;;;
(defsubst rand0 (n)
...
(defun-maybe randij (i j)
  ...
  (cond
   ...
   ((= i j) i)
   ((> i j) (+ j (rand0 (1+ (- i j)))))
   (t	    (error "randij wierdness %s %s" (ti::string-value i) (ti::string-value j)))))
...
;;; ----------------------------------------------------------------------
;;;
(defsubst ti::month-to-number (month)
  "Convert MONTH, 3 character capitalized month name e.g. `Jan' to number."
  (cdr-safe (assoc month  (ti::month-mm-alist))))
...
(defun ti::date-list-regexp (&optional cat-str)
  "Return date regexp.
There is intentional SPACE, since short month name is supposed
...
;;; ----------------------------------------------------------------------
;;; #todo: use read-char-exclusive 19.29+
...
;;;       It does not implement LIST of choices to accept.
;;;
  ...
  (let* (ch)
    ...
    (list
     ...
     (if (or (not (ti::listp list))
	     ...
	     (prog1 nil (ti::xe-character-define-macro 'characterp 'integerp))
     ...
     (while (or (symbolp (setq ch (ti::read-char-safe prompt)))
		(null ch)
		(not (char-in-list-case ch list))))))
    (message "")
    ch))

;;; ----------------------------------------------------------------------
;;; This will also remove the properties:
;;;
;;;   (format "%s" string)
;;;
...
;;;   01 Feb 1996, rainer@physik3.gwdg.de (Rainer Blome)
;;;
(defmacro-maybe applycar (function-form list-form)
...
Examples:
  ...
  -->  (nil nil nil nil nil) ;; global - set - key returns 'nil
    ...
    (applycar (lambda (a b) (list b a)) ;; swaps arguments
      '((1 2)(3 4)))
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::empty-buffer-p (&optional buffer)
...
;;;    (eval-and-compile (ti::d! type common emacs-major message))
	...
	(when (emacs-p)			;XEmacs bindings and we're in Emacs
	  (if message (message message))
	  (ti::ck-advice-control mode)
	  t))
       ((memq type '(emacs emacs-mouse))
	(when (xemacs-p)		;Emacs bindings and we're in XEmacs
	  (if message (message message))
	  (ti::ck-advice-control mode)
	  t))
       (t
...
;;; ----------------------------------------------------------------------
;;; 11 Oct 1996, Kim-Minh Kaplan <kimminh.kaplan@eunet.fr>
;;;
(defun ti::xe-face-p (face-symbol)
  ...
  (cond
   ((emacs-p)
    (or ;; (and (fboundp 'x-display-color-p)
     ...
     (ti::xe-window-system)  ;; Under 21, no colors in tty
     (emacs-p "21")))
   ((xemacs-p)
...
font-lock.el"
  (when (and (featurep 'font-lock)
    ...
    (ti::funcall 'font-lock-mode mode)
    t))

;;; ----------------------------------------------------------------------
;;;
(defun-maybe turn-on-font-lock-mode-maybe ()
...
(defun ti::idle-timer-supported-p ()
  ...
  (and (fboundp 'run-with-idle-timer)
       (or (emacs-p);; Idle timers work in all Emacs versions Win32/Unix
...
;;;   before used
;;; - I chose to keep these names as possible, they consist of 3 char groups:
;;;   BUF(fer)  MAT(ching)
...
;;; ----------------------------------------------------------------------
;;;
(defsubst ti::kill-control-characters ()
...
Return:
  ...
  (` (if (and (not (null (, ch)))	;it must not be nil
	      (or (char-in-list-case (, ch) '(?\t ?\n ?\r ?\f))
		  ;;  esh-mode.el makes wrong definition of
		  ;;  `char-int'. Fix it.
		  (prog1 t
		    (ti::xe-character-define-macro 'char-int   'identity))
		  (and
		   (> (char-int (, ch)) 31)
		   (< (char-int (, ch)) 127))))
	 t nil)))
...
;;; ----------------------------------------------------------------------
;;; By Noah Friedman <friedman@prep.ai.mit.edu>
;;; in his eldoc.el lisp programming aid (post command hook)
;;;
...
something in the echo area."
  (and
   ...
   ;;  Somehow this isn't quite doing what I want. When I test this
   ;;  in TinyLoad ef with C-x C-f open, it still goes on loading
   ;;  while this function should tell "user is in minibuffer"

   (not (eq (selected-window) (minibuffer-window)))

   ;;  This has been disabled because user may move aways from the minibuffer
   ;;  but the minibuffer still stays active there.
...
;;; ----------------------------------------------------------------------
;;; - Very useful when you can accept single or list argument and treat it
;;;   as list in mapcar
;;;
...
;;; ----------------------------------------------------------------------
;;; 31 Oct 1995, jsc@kontiki.sun.com (John Cooper), gnu.emacs.help
...
;;;*	(t (list l))))
;;;
;;; miracle@informatik.uni-oldenburg.de (Jan Schormann)
;;; 1 Nov 1995, gnu.emacs.help replied with non recursive solution
;;; - slightly layout modified by jari
;;;
;;;
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::byte-compile-running-p  ()
  "Check if byte compilation is running on current buffer."
...
do much harm."
  (`
   (eval-and-compile
     ...
     ;;
     (unless (ti::byte-compile-running-p)
...
Example:
  ...
  (when (emacs-p);; Keymaps in XEmacs are not lists
...
;;; ----------------------------------------------------------------------
;;; 1997-04-19 comp.emacs kyle_jones@wonderworks.com (Kyle Jones)
;;;
...
;;;
(defun ti::add-hooks (hook-or-list single-or-list &optional remove append)
...
;;; ----------------------------------------------------------------------
;;; Thu, 15 Feb 96, "Christopher J. Madsen" <ac608@yfn.ysu.edu>
;;;
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::xe-load-user-init-file ()
...
;;; ----------------------------------------------------------------------
;;;
(defmacro ti::xe-Info-directory-list ()
  "Emacs and XEmacs compatibility."
...
;;; ----------------------------------------------------------------------
;;; 28 Aug 1997 wmperry@aventail.com (William M. Perry) in XEmacs beta list
...
(defsubst ti::file-name-path-p (file)
  "Check if file looks like a pathname."
  (string-match "[\\/]" file))
...
(defmacro ti::write-file-as-is-macro (&rest body)
  "Write file without any conversions."
  (`
...
Return:
  ...
  (let* (
	 ;;   This mode does not run any hooks.
...
can be in any file *form* that packages allow for loading."
  (let* (buffer)
    ...
    (with-current-buffer buffer
      (if (and (xemacs-p)		;XEmacs compatibility
	       (fboundp 'eval-buffer))
...
;;; ----------------------------------------------------------------------
;;;
(put 'ti::save-buffer-modified 'lisp-indent-function 0)
(put 'ti::save-buffer-modified 'edebug-form-spec '(body))
(defmacro ti::save-buffer-modified (&rest body)
  "This FORM save modified state during execution of body.
  ...
  (`
   (defgroup (, symbol) nil
     (, doc)
       ...
       ;; You could also use (url-link "mailto:jari.aalto@poboxes.com)
...
In XEmacs ~/.xemacs/config is preferred first."
  (dolist (dir (list
		...
		"~/tmp"
		"~"
		"c:/"))   ;; In case HOME is not set ("~" did not expand)
...
In packages, when definign a config file location, it is usually wanted
that all packages save configuration files to the same location, so that
it it not needed to configure each packages' file manually. The following
code sippet is to show how package can define the configuration
files in a bad and good manner:
...
Input:
  ...
  (when tinylib-:package-config-file-directory
    ...
    (format "%s%s%s%s%s"
	    ...
	    (if emacs
		(format "%s-%s-"
			(if (emacs-p) "emacs" "xemacs")
			(emacs-version-number-as-string))
	      "")
	    (or file ""))))
