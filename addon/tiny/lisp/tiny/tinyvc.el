...
;;; @(#) $Id: tinyvc.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        tools
;;
;; To get information on this program use ident(1) or do M-x tinyvc-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyvc|Jari Aalto|jari.aalto@poboxes.com|
;; RCS rlog minor mode for vc.el, co, un/lock, status, chmod, hilit, mark...|
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinyvc.el|
...
;;; .......................................................... &v-mode ...
;;;  We use name "vc log", bacause this minor mode is called
;;;  mostly from vc
...
(defun tinyvc-select-backend ()
  ...
  (interactive)
  (let* ((ibuffer  tinyvc-:invoked-buffer)
	 file
	 type)
    (when (and
	   ibuffer
	   (get-buffer ibuffer)
	   (setq file
		 (with-current-buffer ibuffer (buffer-file-name))))
      (if (fboundp 'vc-buffer-backend)	;19.30+
	  (setq type (ti::funcall 'vc-buffer-backend))
	;;  nope; that function does not exist. (19.28)
...
Optionally UNMARK. VERB."
  ...
  (let* ((list (tinyvc-get-tmp-list (tinyvc-get-filename)))
	...
	(when (re-search-forward re nil t)
	  ...
	  (cond
	   ((ti::xe-window-system)	;Windowed -- use colors
