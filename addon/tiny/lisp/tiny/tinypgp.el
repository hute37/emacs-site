...
;;; @(#) $Id: tinypgp.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tinypgp-version
...
;;
;;  Brian A. LaMacchia  <bal@martigny.ai.mit.edu>
...
Release note
  ...
  raised by the reports --maintainer.

General

  Please send a brief mail to maintainer if you use TinyPpg and I'll let
  you know about updates. My pgpkey can be fingered at ssjaaa@uta.fi, or by
  sending subject 'send pgpkey.txt' to <jari.aalto@poboxes.com>

  (2.93)  NT Emacs pgp 5.x support work is under way (my 10G disk crashed,
	  and gettings things as they were takes time)
  (2.91)  Fixed some problems with SEMI release 1.8.5. Better MIME
          autodetect support. New feature: If there are both
          non-PGP and PGP powered users in list of recipients, then
          it is possible to send PGP encrypted mail to PGP powered ones
          and plain text to non-pgp people. User is consulted in these cases.
          (applies to auto action only)
          PGP 5.x support is still unresolved.

  (2.70)  PGP 5.x is broken in Unix. Auto-action has now capability
          to send plaintext to people in CC list that don't have
          pgp and encrypted messages for those that do have pgp.

  (2.60)  Emacs 20 Multibyte problems corrected.
          Files used to corrupt when writing them to disk.

  (2.48)  New commands to call PGP/MIME.
          C-c / q  to sign  and  C-c / Q  to encrypt whole mail
          Requires SEMI or TM mime package to be active in buffer
          where you compose mail.

  (2.45)  Basic PGP 5.x support was adjusted in Unix

  Changes in variables

          >> tinypgp-:pubring-table's structure has changed.
          >> tinypgp-:file-secring's  structure has changed.

  (2.33)  Program has undergone big changes in core to introduce PGP 5.
          Basic PGP 5 commands are under work, but don't expect
          anything more yet. The PGP 5 support is only at _BETA_ level.

  (2.24)  WinNT supported has been started. PGP 2.6.x/cmdproxy.exe is expected.
          However due to PC command line argument restrictions, not all
          flags can be used as it would in Unix. It is required that
          there is no pgp config file that would change language or
          verbose or armorlines settings. SE MANUAL FOR NT INSTALLATION.

  (1.345) Version log deleted

General

  While reading this message the TinyPgp.el is already fully operational.
  Try turning on `tinypgp-mode' with \\[tinypgp-mode]

  The latest manual can be found at:

	http://tiny-tools.sourceforge.net/

  I regret that you have to read this message, but as long as this package
  is updated often, I have taken measures to inform possible newbies. If you
  know enough emacs lisp, you can suppress this initial, one time only,
  message.  Once the message has been displayed, you can reload TinyPgp
  multiple times and you won't see it again unless you call
  \\[tinypgp-initial-message]

Using unofficial X-pgp header signing

  1999-09 Note: restrict use of X-pgp and use PGP/MIME instead.

  Refer to section 'Using PGP headers' from the manual before using X-Pgp
  feature. The PGP/MIME has finally started gained firm ground and the Mail
  Agents have started supporting it more widely. The X-pgp was meant to be
  used as an intermediate method until PGP/MIME was widely recognized.

  You may still find use for X-Pgp eg when you post to closed mailing lists
  when blocky PGP/MIME is not looked with good eye. X-Pgp is compact
  digital signature in those cases.

Errors

  IMPORTANT: Install TinyMail and add the example X-info setup from
  the end of package. That will send to me your tiny*el library
  configuration, when you compose mail to me. Otherwise I would have to
  ask many unnecessary questions.

  Expect that you get errors from TinyPgp from time to time and to help me
  to catch the source, you must know how to send me good bug reports. Please
  read he manual how you send good bug reports.

  Also be sure to turn on `tinypgp-:debug' immediately when you get and
  error. Repeat the task and then hit \\[tinypgp-submit-bug-report] which
  will automatically copy the debug information to me.

  The debug buffer *tinypgp-debug* adds cumulatively data to it, so make sure
  it is cleared before you start an 'error session'. That way the debug
  info contains straight transactions between BEGINNING of session and END,
  in this case the ERROR.

  Contact me immediately when you find anything, because I may already have
  new version of this program running and if there is fortunate star on the
  sky, the error may have been noticed and fixed already.
...
Input:
  ...
  (interactive
   (let* ((key-list
     ...
     (setq pass (ti::xe-read-password "Pass phrase: "))
     (if (ti::nil-p pass)
...
(defun tinypgp-timer-control (&optional remove verb)
  ...
  (let* ((fid   "tinypgp-timer-control: ")
    ...
    (ti::xe-timer-cancel-function 'tinypgp-timer-process)
...
(defun tinypgp-password-get  ()
  ...
  (let* ((sym tinypgp-:user-now)
    ...
    (unless (setq ret (ti::vector-table-property tinypgp-:hash-password sym 'password))
 ...
 t     if password available"
  (let* ((fid "tinypgp-password-set:")
	...
	(setq ret t)				;Ok, was in hash
      ...
      (unwind-protect
	  (progn
	    (setq pass (ti::xe-read-password prompt))
...
(defun tinypgp-install-send-mail-hook (&optional remove)
  ...
  (let* (hook
    ...
    (unless remove (ti::add-hooks hook func)) ;Add the hooks in right order
    ))
...
(defun tinypgp-install-hooks-vital (&optional remove)
  ...
  (let* (func
    ...
    (unless remove (ti::add-hooks 'tinypgp-:find-by-guess-hook (reverse list)))
	...
	(add-hook hook func 'append)	;Make sure it is last
	))
  ...
  (when (and (featurep 'vm)
	...
	(dolist (buffer (buffer-list))
	  ...
	  (when (and (eq major-mode 'vm-summary-mode)
	    ...
	    (ti::xe-set-mode-line-format val)
...
(defun tinypgp-mode-define-menu ()
  ...
  (easy-menu-define
   tinypgp-:mode-menu
   (if (xemacs-p) nil (list tinypgp-:mode-map))
   "TinyPgp menu"
   (list
    tinypgp-:mode-menu-name
...
(defun tinypgp-mode-define-keys ()
  ...
  (let* ((map  tinypgp-:mode-map)
	...
	(define-key map (concat p "r")
	  (definteractive (ti::menu-menu 'tinypgp-:region-echo-menu arg)))
	...
	(define-key map (concat p "u")
	  (definteractive (ti::menu-menu 'tinypgp-:user-echo-menu arg)))
      (tinypgp-mode-define-keys-user map (concat p "u")))


    (if tinypgp-:pubring-echo-menu-use-p
	(define-key map (concat p "p")
	  (definteractive (ti::menu-menu 'tinypgp-:pubring-echo-menu arg)))
	...
	(define-key map (concat p "k")
	  (definteractive (ti::menu-menu 'tinypgp-:key-echo-menu arg)))
	...
	(define-key map (concat p "b")
	  (definteractive (ti::menu-menu 'tinypgp-:show-buffer-echo-menu arg)))
	...
	(define-key map (concat p "x")
	  (definteractive (ti::menu-menu 'tinypgp-:extra-echo-menu arg)))
	...
	(define-key map (concat p "m")
	  (definteractive (ti::menu-menu 'tinypgp-:mode-echo-menu arg)))
	...
	(define-key map r
	  (definteractive (ti::menu-menu 'tinypgp-:remail-echo-menu arg)))
    ...
    (if tinypgp-:newnym-echo-menu-use-p
      (define-key map n
	(definteractive (ti::menu-menu 'tinypgp-:newnym-echo-menu arg)))
...
(defun tinypgp-key-mode-define-menu ()
  "Define menus."
  (easy-menu-define
   tinypgp-:key-mode-menu (if (xemacs-p) nil tinypgp-:key-mode-map)
...
(defun tinypgp-summary-mode-define-menu ()
  "Define menus."
  (easy-menu-define
   tinypgp-:summary-mode-menu (if (xemacs-p) nil tinypgp-:summary-mode-map)
...
(defun tinypgp-newnym-mode-define-menu ()
  "Define menus."
  (easy-menu-define
   tinypgp-:newnym-mode-menu (if (xemacs-p) nil tinypgp-:newnym-mode-map)
  ...
  (let* ((tbl tinypgp-:newnym-cmd-table)
	...
	(skip-chars-forward " \t"))
       ...
       ((and (ti::nil-p word)		;User wrote [+-] and wants to complete
	     (char-in-list-case (preceding-char) '(?+ ?-)))
	...
	(if (not (ti::nil-p word))
       ...
       ((and (not (ti::nil-p word))
	     ...
	     (char-in-list-case (following-char) '(?\ ?\t ?\n)))
	(message (nth 2 elt)))
	...
	(cond
	 ...
	 ((symbolp pgp-key)
	  ...
	  (with-current-buffer tinypgp-:buffer-tmp-shell
	    ...
	    (when (or (null list)
		      (not (string-match
			    (replace-regexps-in-string ".*@" "" email 0)
...
(defun tinypgp-nymserver-newpassword (alias password)
  ...
  (interactive
   (progn
    ...
    (list
     (tinypgp-nymserver-ask)
     (ti::xe-read-password "New nymserver password: "))))
...
Input:
  ...
  (let* ((cat  (if (eq (tinypgp-backend-type) 'win32)
	...
	(if (or nil			;Enabled now!
	  ...
	  ;;  PGP gets the password from file desriptor 3. This way
...
Return position of result in buffer.
  ...
  (let* (ret
    ...
    ;;  Pass phrase is good.  Just a moment....
    ;;  Key for user ID: Jari Aalto <jari.aalto@poboxes.com>
...
;;; .
;;; Good signature from user "Jari Aalto <jari.aalto@poboxes.com>".
...
(defsubst tinypgp-send  (string)
  ...
  (expect-send (concat string (if (win32-p) "\r" "\n"))))
	...
	((string-match "#SOURCE-FILE" cmd)
	 ...
	 (setq cmd   (ti::replace-match 0 nil cmd)   ;; Delete tag from command
	   ...
	   (with-expect process
	     ...
	     (unless (win32-p)		;; Unix is slower than NT, add delay
	       (sit-for 0.3))
...
Interactive call note:
  ...
  (let* ((fid	    "tinypgp-key-find-by-http-url: ")
    ...
    (cond
     ((and (not (bufferp stat)) verb)
      (message "Http internal error: %s" stat)
      (sit-for 2)
      (discard-input))

     ((bufferp stat)
      (pop-to-buffer stat) (ti::pmin)
      (if (setq
	   stat
	   (cond
...
Return:
  ...
  (let* ((bcmd	(tinypgp-binary-get-cmd 'key-info))           ;;base command
    ...
    (cond
     ((and (string-match "[ \t]" string) (win32-p))
...
Return:
  ...
  (interactive
   (progn
     (ti::xe-activate-region)	;Make sure user sees region
...
;;;	  (insert sig-b-line "\n")
	  (dolist (elt info-list)
	    (setq elt (replace-regexps-in-string (concat "^" pbase) "" elt))
	    (insert elt "\n"))
...
(defun tinypgp-ask-secring-password (&optional force)
  ...
  (let* ((sym   'tinypgp-:hash)
    ...
    (when (or force (null ret))
      (setq ret (ti::xe-read-password "Password for secring: "))
...
Previous FILE is deleted. VERB."
  (interactive
   (progn
	...
	(ti::xe-read-password "Backup password: ")))))
...
compressed, and uuencoded in base64."
  ...
  (let* ((orig-opt options)
    ...
    (when (and (null orig-opt) (not (win32-p)))
      (setq options (format "+comment=\"%s\"" comment)))
    ...
    (when (and (null orig-opt) (win32-p))
      (tinypgp-binary-header-field-set "Comment:" comment)))
...
Input:
  ...
  (interactive
   (list
    (ti::xe-read-password "Crypt password: ")
    current-prefix-arg))
  (let* ((beg  (ti::mail-text-start))
	 (end  (point-max))
