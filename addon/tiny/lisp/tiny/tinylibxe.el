...
;;; @(#) $Id: tinylibxe.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tilibxe-version
...
(ti::fboundp-check-autoload 'button-release-event-p "tinylibxe"
  ...
  (defun button-release-event-p (event)
    ...
    (and (eventp event)
	 (memq (ti::funcall 'event-basic-type event) '(mouse-1 mouse-2 mouse-3))
...
(when (and nil (emacs-p)) ;; disabled now
...
(when (xemacs-p)
  ...
  ;; ----------------------------------------------------------------------
  ;; by Anders Lindgren <andersl@csd.uu.se>
  ;;
