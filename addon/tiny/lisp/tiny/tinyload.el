...
;;; @(#) $Id: tinyload.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    extensions
;;
;; To get information on this program use ident(1) or do M-x tinyload-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyload|Jari Aalto|jari.aalto@poboxes.com|
;; Queue list of packages to load until emacs is idle. Kinda lazy load/require|
;; 2002-05-06|$Revision: 1.1 $|~/misc/|
...
;;     (require 'tinyload)
;;
;; TinyLoad can't be autoloaded, because it installs one idle-timer
;; function.
;;
...
(ti::macrof-debug-standard "tinyload" "-:")
...
the list pointer to 0."
  ...
  ;;  Kill old process(es)
  (ti::xe-timer-cancel-function 'tinyload-loader-process)
...
(defun tinyload-autoload-function-load (&optional verb)
  ...
  (let* ((fid "tinyload-autoload-function-load:")
	 (funcs (ti::y-autoload-function-list))
	 (load (when funcs
		 (ti::y-autoload-function-file-list funcs)))
	 (count 0)
	 str)
    (tinyload-debug
...
Return:
  ...
  `tinyload-:load-list'"
  (let* ((id "tinyload-config-file-parse")
	 list
	 test
	 file)
    (ti::pmin)
...
Return status '(continue no-action no-input)."
  (let* ((id "tinyload-process-continue")
	 no-action
	 no-input
	 continue)
...
(defun tinyload-eval (form type)
  ...
  (condition-case err
    ...
    (error
     (let* ((str
	     (format "Tinyload: EVAL %s generated error %s %s"
		     type
		     (prin1-to-string err)
...
(defun tinyload-load-failure (pkg elt)
  ...
  (let* ((failed-list (get 'tinyload-:load-list 'failed-list)))
    ...
    (let ((str
	   (format "TinyLoad: [ERROR] loading %s" pkg)))
      (ding)
      (tinyload-debug str)
      (tinyload-message str))
...
;;	$Id: tinyload.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
;;
;;	This is one of my personal Emacs (rc) resource files and it
...
(defconst tinyload-:load-list
  (list
   ...
   (when w
     (list (if (emacs-p) "emacs-rc-font" "emacs-rc-font.el")
	   'rc-font nil nil nil
   ...
   (unless nt  ;; I don't use mail in NT
   ...
   ;;	My primary emacs at the time of writing this package.
   ;;   This loads some overloaded and overriden Emacs functions.
   ;;   Emacs needs few fixes...
   ...
   (when (and w (emacs-p))
     (list "emacs-rc-x-menu"))

   ;;  Don't know if this is really necessary, no-op if it is not.

   (when (and w (emacs-p))
     (list "scroll-bar" nil 'noerr))

   (list "tinypgp" nil nil nil
	 ;;   I dont' want to see startup screen
	 ;;
	 '(progn (provide 'tinypgp)))

   (list "emacs-rc-tipgp") ;Second generation Emacs PGP interface
   ...
   ;;  My personal lisp function library. Run compression
   ...
   (list "mylib.el"
	 ...
	 '(progn
	    (when (fboundp 'my-compress-household)
	      (ti::funcall 'my-compress-household))))

   ;;  Make sure we have newest advice.

   (if (file-exists-p (ipath "advice.el"))
       (if (or (not (boundp 'ad-version))
	       (not (string=
		     (symbol-value 'ad-version)
		     "2.15")))
	(list (ipath "advice")))))))
...
;; (eval-when-compile
;;   ;;   If we use compressed files, we can't use autoload
;;   (if (require 'tinyload)
;; 	 (autoload 'tinyload-install "tinyload" t t)))
;;
;; ;;  It may be under these two features, depending on my current system
