...
;;; @(#) $Id: tinylpr.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:     extensions
;;
;; To get information on this program use ident(1) or do M-x tinylpr-version
;; Look at the code with folding.el, tinyfold.el, tinybm.el

;; LCD Archive Entry:
;; tinylpr|Jari Aalto|jari.aalto@poboxes.com|
;; Easy Emacs lpr command handling, popup, completions|
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinylpr.el.Z|
...
;;	(global-set-key "\C-zp" (definteractive (ti::menu-menu 'tinylpr-:menu)))
;;
...
;;            (interactive "e")
;;            (let* ((pstat
;;                 (replace-regexps-in-string           ;remove directory name
;;                  ".*/"
;;                  ""
;;                  (or (my-print-status) "")))
;;                item)
