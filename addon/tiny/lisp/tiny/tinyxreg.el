...
;;; @(#) $Id: tinyxreg.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    tools
;;
;; To get information on this program use ident(1) or do M-x tinyxreg-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyxreg|Jari Aalto|jari.aalto@poboxes.com|
;; Stores point/win cfg to X-POPUP LIST while storing it to register|
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinyxreg.el.Z|
...
;;
;; If you don't want default bindings, put this before require or
;; autoload commands.
;;
;;* _
;;* 	(setq tinyxreg-load-hook nil)
;;
;; If you have any questions, comments, use this function
;;
...
(defun tinyxreg-event ()
  "Return fake event."
  (ti::xe-make-fake-event tinyxreg-:x-coord tinyxreg-:y-coord))
...
Input:
  ...
  (let* ((event  (or event
    ...
    (cond
     ((null (ti::xe-window-system))
      (message "TinyXreg: sorry, Requires X to use X-popup"))

     ((null list)
      (if verb (message "TinyXreg: sorry, both register lists are empty.")))

     (t
      (setq data (ti::xe-popup  list event nil title))
      (if (null data)
	  (if verb (message "TinyXreg: register not selected."))
