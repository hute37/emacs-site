...
;;; @(#)$Id: tinyhotlist.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:     tools
;;
;; To get information on this program use ident(1) or do M-x tinyhotlist-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyhotlist|Jari Aalto|jari.aalto@poboxes.com|
;; Hotlist of important buffers, files(ange-ftp), dired: easy add, easy remove|
;; 2002-05-07|$Revision: 1.1 $|~/misc/tinyhotlist.el.Z|
...
Returns:
  ...
  (let* (buffer-file
	...
	(when buffer-file  	;;  Get the directory name
	  (let* ((abbrev (tinyhotlist-abbreviate-file-name
	    ...
	    (push (cons elt buffer-file) tinyhotlist-:cache)))))
...
Return:
 ...
 nil t"
  ...
  (let* ((file  tinyhotlist-:hotlist-file)
	...
	(setq tinyhotlist-:cache (reverse list))
	t))
  ...
  (let* ((list   (mapcar (function car) tinyhotlist-:cache))
	 ...
	 (x      (cond
		  ...
		  (t
		   nil))))			   ;; no X available...
    (if x
	(ti::xe-popup list event nil title)
...
(defun tinyhotlist-control-kbd (&optional arg)
  ...
  (interactive "P")
  (tinyhotlist-control
   (ti::xe-make-fake-event tinyhotlist-:x-coord tinyhotlist-:y-coord) arg))
...
Optional ARG can be:
  ...
  (let* ((buffer (buffer-name))
	...
	(sleep-for 1))
       ...
       ;;  I hear you: "Why number 9??" --   Because it's next to number 0
       ((eq 9 arg)
	(tinyhotlist-set-defaults)
	...
	(message "TinyHotlist: Nothing to rewmove, [%s] wasn't in hotlist."
		 buffer)))
