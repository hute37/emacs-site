...
;;; @(#) $Id: tinyprocmail.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    extensions
;;
;; To get information on this program use ident(1) or do M-x tinyprocmail-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinyprocmail|Jari Aalto|jari.aalto@poboxes.com|
;; (p)roc(m)ail minor mode. Browse and manage procmail log files.|
;; 2002-01-30|$Revision: 1.1 $|~/misc/tinyprocmail.el.Z|
...
;;	;;  This is not strictly necessary, because package will determine the
;;	;;  version number automatically when loaded for the first time.
;;
...
;;
;;  With this packege there is sample procmail test file for Lint. You can
;;  unpack it if you have `pgp' and `tar' commands in your system.  When the
;;  file unpacks load the *rc file into buffer, and follow instructions in
;;  the file.
;;
...
(eval-and-compile
  ...
  (autoload 'tinycompile-parse-line	    "tinycompile" "" t)
  (autoload 'turn-on-tinytab-mode	    "tinytab"     "" t)
  (autoload 'turn-off-tinytab-mode	    "tinytab"     "" t)
  (autoload 'tinytab-mode		    "tinytab"     "" t)
  (autoload 'timy-vi-type-paren-match	    "tinymy"      "" t)
  (autoload 'timy-backslash-fix-paragraph   "tinymy"      "" t)
...
This function must standardize the flag order by calling
...
(defcustom tinyprocmail-:procmail-version (tinyprocmail-procmail-version)
  "The version string '3.11pre7' returned by `procmail -v'.
procmail v3.11pre7 1997/04/28 written and created by Stephen R. van den Berg"
  :type 'string
  :group  'TinyProcmail)
...
(defcustom tinyprocmail-:font-lock-keywords
  (list
   ...
   (list
    (concat
     "[^_a-zA-Z0-9]\\("
     "VERBOSE\\|DELIVERED\\|COMSAT\\|LOG\\|EXITCODE\\|"
     "LOGFILE\\|LOGABSTRACT\\|MAILDIR\\|HOST\\|"
   ...
   (list
    (concat
     ...
     "test\\|formail\\|sendmail\\|"
     "sed\\|head\\|tail\\|cat\\|echo\\|"
     "awk\\|perl"
     "\\)[^_a-zA-Z0-9]")
    1 'font-lock-reference-face)
...
(ti::macrof-minor-mode-wizard
 "tinyprocmail-" " PM" "\C-c'"  "PM" 'tinyPm "tinyprocmail-:"
...
\\{tinyprocmail-:mode-prefix-map}"
  ...
  (list                                  ;arg 10
   ...
   (list
    ...
    ["Fix backslashes at point" tinyprocmail-fix-backslashes-region	t]
    ["Match parenthesis"	timy-vi-type-paren-match		t]
  ...
  (progn
    ...
    (define-key	  map "%"	'timy-vi-type-paren-match)
    (define-key	  map  "\\"	'tinyprocmail-fix-backslashes-region)
    ...
    (define-key	  map "\C-n"		'tinyprocmail-forward)
    (define-key	  map "\C-p"		'tinyprocmail-backward)
    (define-key	  map [(end)]		'tinyprocmail-forward)
    (define-key	  map [(home)]		'tinyprocmail-backward)
    (define-key   map [(control end)]   'tinyprocmail-forward-strict)
    (define-key   map [(control home)]  'tinyprocmail-backward-strict)
...
(ti::macrof-minor-mode-wizard
 "tinyprocmail-output-" " PM-Lint" "\C-c'"  "PM-Lint" 'tinyPm "tinyprocmail-output-:"
...
\\{tinyprocmail-output:mode-prefix-map}"
  ...
  (progn
    (when (and (interactive-p)		;On when user calls us directly
	       ;; Mode is Now turned on, check Lint buffer adn confirm
	       tinyprocmail-output-mode
	       (null (ti::re-search-check (concat "^" (regexp-quote "*** "))))
	       (null (y-or-n-p "No Lint output found. Are you sure? ")))
  ...
  (list                                  ;arg 10
   ...
   ["Find error"		tinycompile-parse-line 	            t]
  ...
  (progn
    (if (emacs-p)
	(define-key map [mouse-2] 'tinycompile-parse-line)
      (define-key map [(button2)] 'tinycompile-parse-line))


    ;;  Define keys like in Compile

    (define-key map  "\C-c\C-c" 'tinycompile-parse-line)
    (define-key map  "\C-m"     'tinycompile-parse-line)
    ...
    (define-key map  "C"        'tinyprocmail-output-clear)
    (define-key map  "?"        'tinyprocmail-output-mode-helpx))))
...
must match `^:0'."
  ...
  (dolist (buffer (buffer-list))
	...
	(if  verb (message "TinyPm: Mode turned %s in %s"
...
(defun tinyprocmail-overlay (point)
  ...
  (when (and (fboundp 'make-overlay)
    ...
    (dolist (elt '((owner tipm)
		   ...
		   (face           highlight)
		   (before-string  ">")))
      (overlay-put tinyprocmail-:overlay  (nth 0 elt) (nth 1 elt)))
    (save-excursion
      (goto-char point)
...
(defun tinyprocmail-overlay-hide ()
  ...
  (interactive)
  (when (and tinyprocmail-:overlay
	     (fboundp 'move-overlay))
    (move-overlay tinyprocmail-:overlay 1 1)
    (overlay-put tinyprocmail-:overlay 'before-string "")))
...
(defun tinyprocmail-move-to-recipe-end ()
  ...
  (let* (beg)
	...
	(tinyprocmail-move-to-macthing-brace 'no-adjust)
	  ...
	  (error "TinyPm: Can't find recipe end.")))
    (cons beg (point))))
...
;;; ----------------------------------------------------------------------
;;;
(defun tinyprocmail-fix-backslashes-region ()
  "Fix backslashes to column 76."
  (interactive)
  (timy-backslash-fix-paragraph
   76
   '(lambda ()
      :; #todo: Later...
      nil)
...
Return:
  ...
  (let* ((opoint (point))
	...
	(if verb
	    (message "TinyPm: No more recipes backward."))
	(goto-char opoint)
	nil))
     (t
      (end-of-line)
	  ...
	  (goto-char (match-beginning 1))
	(if verb
	    (message "TinyPm: No more recipes forward."))
	(goto-char opoint)
	nil)))))
...
(defun tinyprocmail-flag-read (string)
  ...
  ;;
  (setq string (replace-regexps-in-string "[ \t]+" "" string))
...
(defun tinyprocmail-flag-format-default (flags)
  ...
  (let* (new)
	...
	(if (not (eq tinyprocmail-:flag-and-recipe-start-style 'flags-together))
	    (concat " " new)
	  new)
...
;;;	(ti::d! "FLAGS" flags1 flags2)

	;;  How id the standardization go?
  ...
  (let* ((supp    (tinyprocmail-supported-p 'condition-middle-comment))
	...
	(looking-at "^[ \t]*\\*.*#"))
      (tinyprocmail-log
       ...
       point-min)
      (tinyprocmail-fix-macro "Comments not allowed in condition, Move upwards "
	(setq no-move
...
;;;    (ti::d! (ti::read-current-line (point)))
    (setq cond-end  (point)
	  flags	    (or flags "")
	  pipe-p    (looking-at "[ \t]*|")
...
;;;	(ti::d! (progn (goto-char redirection-p) (ti::read-current-line)))
	...
	(if (null point)
      ...
      (ti::pmin)
      (when (and pedantic
		 (setq point (tinyprocmail-lint-condition-comments point-min)))
	(tinyprocmail-log
	...
	(when (string-match "H" flags)
	  ...
	  (tinyprocmail-fix-macro (concat str " Remove ")
	    (setq fix-line  t
		  flags     (replace-regexps-in-string "H" "" flags))))
	(when (string-match "B" flags)
	  (setq str "Error, size test doesn't use `B' flag. (use B ?? <)")
	  (tinyprocmail-log size-test-p str)
	  (tinyprocmail-fix-macro (concat str " Remove ")
	    (setq fix-line t
		  flags (replace-regexps-in-string "B" "" flags))))))
	...
	(tinyprocmail-fix-macro (concat str " Remove ")
	  (setq fix-line t
		flags (replace-regexps-in-string "c" "" flags))))
	...
	(tinyprocmail-fix-macro (concat str " Remove ")
	  (setq fix-line t
		flags (replace-regexps-in-string "i" "" flags))))
	...
	(tinyprocmail-fix-macro (concat str " Remove ")
	  (setq fix-line t
		flags (replace-regexps-in-string ":" "" flags))))
	...
	(tinyprocmail-log opoint "Error, formail -D used. Flag `f' is a mistake.")
	(tinyprocmail-fix-macro "Formail -D used, remove `f' flag? "
	  (setq fix-line t
		flags (replace-regexps-in-string "f" "" flags))))

      (when (not (string-match "W" flags))
	(tinyprocmail-log opoint "Warning, formail -D used but no `W' flag found.")
	...
	(tinyprocmail-log opoint "Error, formail -D used. SHould not have `b'.")
    ...
    (when (and (ti::string-match-case "f" flags)
      ...
      (tinyprocmail-fix-macro (concat str " Correct ")
	(setq fix t)
	(setq flags (replace-regexps-in-string "c" "" flags))))
...
Warning, no \">\" in \"|\" recipe 'i' kills message. May need `c'.")
    ...
    (when (and flags
      ...
      (tinyprocmail-fix-macro (concat str " Correct")
	(setq fix-line t
	      flags (replace-regexps-in-string "W" "" flags))))
    ...
    (when (and mbox-p
	       ...
	       (not forward-p)
	       (not lock-p))
      (setq str "Warning, message dropped to folder, you need lock.")
...
Warning, flag combo `hb' is useless when dropping to folder.")
	...
	(tinyprocmail-fix-macro (concat str " Remove ")
	  (setq fix-line t
		flags (replace-regexps-in-string "hb\\|bh" "" flags)))))
    ...
    (when (looking-at "[ \t]*:0[^:\n]*\\(:\\)\\( [a-z]\\)[a-z]?[ \t\n#]+")
      (setq str (save-match-data (ti::string-remove-whitespace (match-string 2))))
...
;;;      (setq cont-p (looking-at ".*[\\][ \t]*$"))
	...
	(when (and (looking-at "^[ \t]*\\([^ \t\n]+\\)[ \t]*=")
	  ...
	  (setq str
		(format
		 "Warning, odd characters in variable name `%s'." str))
	  (tinyprocmail-log (point) str)
	  (tinyprocmail-fix-macro str))
	...
	;; D = ${D: ...
	;;
	;; Note that the colon in this case is ok.  D = {D}:/directory/file.txt
	...
	(when (looking-at
	  ...
	  (setq var1 (match-string 1)
		...
		str  (match-string 4)
		op   (match-string 5))
;;;	(ti::d! 11334 var1 "v2" var2  "op" op "s" str)
	  ...
	  (when (save-match-data (ti::nil-p var2))
	    (tinyprocmail-log (point) "Error, no right hand init variable found.")
	    (tinyprocmail-fix-macro
	  ...
	  (when (save-match-data (and var2 (string-match "^[$]" var2)))
		...
		(format "`%s' contains extra $. Correct " var2)
	      (save-match-data (setq
				var2
				(replace-regexps-in-string "^[$]" "" var2)))
	  ...
	  (when nil
	    (when (and (save-match-data (looking-at ".*[-+][ \t]*`"))
	      ...
	      (tinyprocmail-log
	       (point)
	       "Pedantic, `` is not a recommended init practise (uses shell).")))
...
(defun tinyprocmail-lint-malformed-start-recipe ()
  "Check ': ' or '0:' recipes."
  (let* ((space  (if (eq 'flags-together tinyprocmail-:flag-and-recipe-start-style)
		     "" " "))
	 list
	 str)
    (while (setq list (ti::re-search-point-list '("^[ \t]*0:" "^[ \t]*:[^0]")
      ...
      (cond
       ((looking-at "^[ \t]*\\(0:\\)")
	(tinyprocmail-log (point) "Error, recipe start is invalid, should be `:0'.")
...
Refrences:
 `tinyprocmail-:flag-and-recipe-start-style'"
  ...
  (let* ((style tinyprocmail-:flag-and-recipe-start-style)
    ...
    (unless (zerop i)
      (message
       "TinyPm: standardized %d recipe start line(s). Style: %s"
       i
       (if style
	   (symbol-name style)
    ...
    (when (zerop found)
      (message "TinyPm: Can't find any recipes from buffer."))))
...
(defun tinyprocmail-lint-find-2spaces ()
  ...
  (let* (str
	...
	(setq set	   (match-string 1)
	      not-comment  (save-match-data (not (tinyprocmail-comment-line-p)))
	      not-backtics (save-excursion
			     (beginning-of-line)
			     (save-match-data (not (looking-at ".*`")))))

;;;	(ti::d! (point) (ti::read-current-line (point)))
...
Input:
  ...
  (let* ((check-list		tinyprocmail-:lint-do-hook)
    ...
    (while (tinyprocmail-forward)
      ...
      (when (and verb mode)
	(message "TinyPm: Linting %d %s" count (ti::read-current-line)))
    ...
    (message "TinyPm: Lint done. recipe Count = %d time: %02d:%02d "
