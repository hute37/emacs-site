...
;;; @(#) $Id: tinygnus.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    extensions
;;
;; To get information on this program use ident(1) or do M-x tinygnus-version
...
(eval-and-compile
  ...
  (let* ((i 0)
	 emacs-gnus
	 error)
    (ignore-errors (require 'gnus-group))
    (unless (ignore-errors (require 'message))      (incf  i))
    (unless (ignore-errors (require 'nnmail))	    (incf  i))
    (unless (ignore-errors (require 'nnml))	    (incf  i))
    (unless (ignore-errors (require 'nnfolder))	    (incf  i))
    (unless (ignore-errors (require 'nnheader))	    (incf  i))

    ;;  mm-util defined mm-char-int, which is used in
    ;;  gnus.el::gnus-continuum-version
    ;;
    ;;  => continuum fails, if mm-char-int is not defined.
    (unless (ignore-errors (load "mm-util"))	    (incf  i))
    ...
    (unless (ignore-errors (require 'gnus-agent))
      (message "Loading gnus-agent failed %s" (locate-library "gnus-agent"))
      (incf  i))
    ...
    (setq error
	  (cond
	   ...
	   (;;  Standard Gnus than comes with Emacs is not accepted.
	    ;;  User must be running development version of Gnus.
	    (setq emacs-gnus
		  (string-match
		   (concat
...
(defcustom tinygnus-:ube-forward-mail-addresses
  '(;; Note: 2000-02-10 not valid any more.
    ;; "spam-list@toby.han.de"
    ;; "uce@ftc.gov"
    )
...
(ti::macrof-debug-standard  "tinygnus" "-:")
...
(defun tinygnus-install (&optional uninstall)
  ...
  (let* ((list  '((gnus-group-mode
    ...
    (dolist (elt list)
      (setq hook (nth 1 elt)  hook-list (nth 2 elt))
...
;;; ----------------------------------------------------------------------
;;; Code by Torsten Krueger <tot@kruger.in-berlin.de>
;;; 2000-07-02 gnu.emacs.gnus <87zoo1vueb.fsf@musica.orfeo.fr>
...
References:
  ...
  (let ((fid  "tinygnus-nslookup-filter")
    ...
    (tinygnus-debug fid "NS-LOOKUP-LIST" (reverse ns-lookup-list))
    (tinygnus-debug fid "NS-LIST" (reverse ns-list))
    (list ns-lookup-list
	  ns-list)))
...
Return:
 ...
 '(err-list ok-list)."
  (let ((fid  "tinygnus-nslookup-do")  ; Function id
	err-list
	(id  "tinygnus-nslookup-do"))
    (dolist (elt (ti::mail-nslookup list nil 'verb))
      (tinygnus-debug id elt)
...
(defun tinygnus-ube-address-compose (ns-list)
  ...
  (let* ((id  "tinygnus-ube-address-compose")
	 str
	 done
	 tmp-list
	 addr-list
...
;;;	(ti::d! done str ip tmp-list)
	...
	(when (stringp (tinygnus-compose-return-address str))
      ...
      (tinygnus-debug id "ADDR-LIST" addr-list)
      addr-list))
...
References:
  ...
  (let* ((message-included-forward-headers ".")
	...
	(setq addr-list (reverse addr-list))
...
;;; ----------------------------------------------------------------------
;;; 97-10-06 wmperry@aventail.com (William M. Perry) in ding mailing list
;;;
...
;;; ----------------------------------------------------------------------
;;; By Steven L Baur <steve@miranova.com>
;;;
...
;;; ----------------------------------------------------------------------
;;; 1999-02-16 Petr Konecny <pekon@decibel.fi.muni.cz> gnu.emacs.gnus
;;; Message-ID: <qwwr9rrdpz9.fsf@decibel.fi.muni.cz>
;;;
  ...
  (let* (nnml-list)
	...
	(cond
	 ...
	 (t
	  (gnus-group-goto-group group)
	  (gnus-group-kill-group)
	  (gnus-group-make-group name server)
	  (gnus-group-set-current-level 1 level)
  ...
  (if (not (boundp 'nnmail-procmail-directory))
    ...
    (when (y-or-n-p "Create many nnml backend folders from spool? ")
      (tinygnus-files-from-dir-macro
       ...
       (ignore-errors
	 (let* ((name (replace-regexps-in-string
  ...
  (let* ((files (directory-files dir nil "^[0-9]+$"))
    ...
    (when list
      (cons (string-to-number (car list))
	    (string-to-number (car (reverse list))) ))))
    ...
    (let ((method (or method (inline (gnus-find-method-for-group group))))
      ...
      (setq
       ...
       (and (inline (gnus-check-server method))
	    ...
	    (unless active
	      (pr "(parse-active)NNTP buffer conatins no data" nntp-server-buffer)
...
(defun gnus-read-newsrc-file (&optional force)
  ...
  ;;jari: Make sure this is defined
  ;;
  (setq gnus-current-startup-file (gnus-make-newsrc-file gnus-startup-file))
