...
;;; @(#) $Id: tinybuffer.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:     extensions
;;
;; To get information on this program use ident(1) or do M-x tinybuffer-version
...
;;*   ;; Here is the HP Numpad keys used
;;*   ;;
...
;;* _
;;*   ;;   Here is alos code to switch between all visible windows should
;;*   ;;   you find it useful.
;;*   ;;
;;*   (global-set-key [(f5)] (definteractive	; in tinylibm.el
;;* 		       (other-window 1 t)
...
;;;      (ti::d! str buffer (char= ch key-back) (char= ch key-fw) go-list)
	...
	(setq buffer (tinybuffer-iswitch-next)))
       ((and ch (char-in-list-case ch go-list))
	(setq loop nil)))
    ...
    (if (and ch
	     buffer
	     (not (char-in-list-case ch quit-list)))
