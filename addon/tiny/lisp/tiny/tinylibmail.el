...
;;; @(#) $Id: tinylibmail.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    extensions
;;
;; To get information on this program use ident(1) or do
;; M-x tinylibmail-version
...
(defvar ti:mt-load-hook nil
  "Hook that is run when package is loaded.")

;;; ....................................................... &v-private ...

(defvar ti:mt-ret nil
  "Global return value of this package.")

(defvar ti:mt-mail-buffer " *ti::mail-mail*"
...
(defconst ti:mt-required-headers
...
(defvar ti:mt-parse-name-not-accept
  (concat
...
(defconst tinylibmail-version (substring  "$Revision: 1.1 $" 11 16)
...
Exceptions:
  ...
  (let* ((end (ti::mail-pgp-signed-conventional-p))
    ...
    (save-excursion
      (cond
       ((and end (null (ti::mail-pgp-signature-detached-p)))
...
(defun ti::mail-mail-mode-p ()
  ...
  (string-match
   "^\\(vm-\\|rmail-\\|mh-\\|gnus-article-\\).*mode"
   (symbol-name major-mode)))

;;; ----------------------------------------------------------------------
;;;
(defun ti::mail-mailbox-p ()
  "Check if two first lines look line Berkeley mailbox format."
  (save-excursion
    (ti::pmin)
...
`my-message-generate-new-buffers' or `mail-setup-hook'"
  ...
  (let ((to  (if (string= (buffer-name) " *gnus article copy*") ;; See gnus-msg.el
	...
	(setq str to)))
      ...
      (setq str (replace-regexps-in-string "['\"]" "" str)) ;remove extra cruft
...
The blocks searched are in following format.
  ...
  Key for user ID: Mr. Foo <foo@site.com>
...
Return:
  ...
  (let ((re  (concat
	...
	(when (and control-list
	  ...
	  (dolist (elt list) (setq blocks (delete elt blocks)))
...
(defun ti::mail-email-from-string (string)
  "Return email addresses from STRING.
...
that the old functionality is preserved in spite of changes."
  (let (list
    ...
    (setq list
	  '("<jari.aalto@poboxes.com> (Finland, pgp id 512/47141D35)"
	    "(Rune Juntti[FRONTEC Pajala]) <Rune.Juntti@pjl.frontec.se>"
	    "shahramn@wv.mentorg.com (shahram_najm@mentorg.com)"
	    "(santini@chambord.univ-orleans.fr)"
	    "Jerome Santini <santini@chambord.univ-orleans.fr>"
	    "halladay@stlhp56.ug.eds.com (Harry Halladay - EDS St. Louis)"
	    "etxaks@aom.ericsson.se (Ake Stenhoff TM/PMD 83442 3003)"
	    "Jari-Teppo Aalto jari.aalto@poboxes.com"
	    "TOMMI <\"VAX::SOME@site.here.com\""
	    "\"VAX::LOGIN\"@site.here.com"
	    "Job.Ganzevoort@cwi.nl"
	    "Marla=Bush%aoa.rdt%OS.DC@Ban-Gate.AoA.DHHS.EDU"
	    "mernst@theory.lcs.mit.edu (Michael Ernst)"
	    "\"/G=Name/S=Surname/OU=comlab/O=oxford/PRMD=UK.AC/ADMD= /C=GB/\"@site.fi\""
	    "\"wayne (w.d.) bell\" <wbell@bnr.ca>"
	    "Ted Stern <stern@amath.washington.edu>"
	    "\"Joseph B. Ottinger\" <joeo@freenet.scri.fsu.edu>"
	    "\"Name Foo puh. 111 600\" <LOGIN@site.fi>"
...
Return:
  ...
  (let* ((re-A		"[-a-zA-Z0-9.{|]")
	 ...
	 (q-no-re   ti:mt-parse-name-not-accept)
...
point movement is not preserved with `save-excursion'.

Comment

    Where do you need this? If you got spam mail, you can use
    this function to find out from where the mail originated and
    send complaint to postmasters of all those sites.
...
Received: from mail.msss.v.com [atl.asd.com [234.454.54]]
  ...
  (let* ((regexp-list (ti::mail-parse-received-regexp-list))
	...
	(setq elt (ti::remove-properties elt))
	(when (and (stringp elt)
	  ...
	  (setq elt (replace-regexps-in-string ".*@" "" elt))
	  (setq elt (replace-regexps-in-string "[]()\n]" "" elt))
...
(defun ti::mail-nslookup-parse ()
  "Parse nslookup output in current buffer.
...
'(NAME.ANSWER.COM (NNN.NNN.NNN.NNN  NNN.NNN.NNN.NNN ..))."
  (let* (name
    ...
    (when (re-search-forward "^[ \t]*$" nil t)
      (forward-line 1)
      (ti::narrow-safe (point) (point-max)
	(ti::pmin)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::mail-nslookup (ip-list &optional options verb)
  "Do nslookup for each address in IP-LIST. VERB.
The response that is parsed from nslookup command must be in format:

  Name:    rita.upc.es
  Address:  147.83.86.38
...
Input:
  ...
  IP-LIST	string, or list of string. The IPs to investigate.
		'(IP IP IP ...)
  OPTIONS	list, additional options to nslookup. Eg. -query=any
...
Return:
  ...
  if nslookup fails; the elt will be '(IP nil)"
  (let* (;;  It's faster to use absolute pathname.
	 ;;
	 (path  (or (get 'ti::mail-nslookup 'nslookup-binary)
		    (executable-find "nslookup")
		    (error "No `nslookup' binary found.")))
	 args
	 ret)

    (put 'ti::mail-nslookup 'nslookup-binary path)

    ;;  Allow single entry too "ip" --> '("ip")

    (setq ip-list (ti::list-make ip-list))
    ...
    (with-temp-buffer
     (dolist (ip ip-list)
       ...
       (when verb
	 (message "tinylibmail: Doing nslookup %s" ip))

       (when (string-match "\\.[0-9][0-9]?[0-9]?$\\|\\.[a-z][a-z][a-z]?$" ip)
	 (setq args options)
	 (push ip args)
	 (apply 'call-process
		path
	 ...
	 (multiple-value-bind (name ip-list)
	     (ti::mail-nslookup-parse)
	   (if name
	       (push (list ip
			   (cons name (car ip-list)))
		     ret)
	     ;; nslookup failed
	     (push (list ip nil) ret))))))
    ret))
...
(defun ti::mail-current-field-name  ()
  "Return name of field on current point or nil."
  (save-excursion
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::mail-field-email-send-p (&optional data header-regexp)
...
Return:
  ...
  (let ((hdr-end  (ti::mail-hmax))
    ...
    (save-excursion
      (when (and hdr-end
		 (setq beg  (ti::mail-field-start field-re 'move))
...
Return:
  ...
  (let* ((field-re (concat "^" field ":")))
    (when text
      (setq text (ti::string-verify-ends text "\n"))
      ;;  There must be leadig space after field keyword.
      (setq text (ti::string-verify-ends text " " nil 'beg)))
    (save-excursion
      (cond
       (look-field

	(if replace (ti::mail-kill-field field-re)) ;Remove
...
Input:
  ...
  (save-excursion
    (ti::pmin)
    (if (null (mail-fetch-field field))
	(ti::mail-add-field field (concat " " string) look-field)
...
Example:
  ...
  (let* (flag
    ...
    (when (setq fld    (ti::mail-get-field field))
      ...
      (setq fld (replace-regexps-in-string "[\n\f\t ]+" "" fld))
...
If NA-MODE is nil: do 'an' conversion
	anXXX@site.com   --> anXXX
	naXXX@site.com   --> anXXX
       VANITY@site.com   --> VANITY@site.com
    VANITY.an@site.com   --> VANITY@site.com
    VANITY.na@site.com   --> VANITY@site.com
...
;;; ----------------------------------------------------------------------
;;; 19 Feb 1996, elgaard@diku.dk (Niels Elgaard Larsen), comp.emacs
;;; #not tested
;;;
...
(defmacro ti::mail-sendmail-macro (to subject send &rest body)
  "(to subject send &rest body) Send/construct mail according to parameters.
Use TO, SUBJECT and If SEND if non-nil, send mail after BODY finishes.

The point is at the beginning of body.  If you want to send pure mail
without mode setting, bind following variables `mail-setup-hook' and
`mail-mode-hook' to nil.

Mail is sent when BODY has been run.
...
Note:
  ...
  (`
   (let* ((BuffeR	(ti::temp-buffer ti:mt-mail-buffer 'clear)))
