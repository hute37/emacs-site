...
;;; @(#) $Id: tinymail.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    tools
;;
;; To get information on this program use ident(1) or run M-x tinymail-version
...
;;   ~/.emacs startup file.
;;   Code can be easily ripped with ti::package-rip-magic from tinylib.el
;;
...
;;* _
;;*     ;; [optional] If you want nice TAB indent for your messages,
;;*     ;; add this too. You TAB advances 4 spaces in the body of message.
...
;;*      "Define my mail things."
;;*      ;;  This key completes anywhere in the mail buffer.
;;*      ;;  Pick faster key than the default installation
;;*      (define-key mail-mode-map [(f5)] 'tinymail-complete-guess))
;;*
;;
...
;;	M-x tinymail-version               [Added C-u will show version only]
...
;;	o   Generate anti-ube addresses to prevent UBE/Spam from arriving to your
;;	    mailbox.
...
;;  To field tracking
;;
;;      While composing message, this package awakens in
...
;;
;;	If you use Gnus and separate spool files, like you would do with Procmail,
;;	then you need to gather mail arrival information from all the spool files.
;;	Let's suppose you don't want to get notified on mailing list messages,
...
;;
;;	Take a look at variable `tinymail-:report-spool-buffer-control' which
;;	has default value 'keep where the `tinymail-:report-mail-notify-program'
;;	results are gathered. You may find it useful to keep the
;;	`tinymail-:report-spool-buffer' *tinymail-mail-spool* visible in some
;;	frame to act like `biff(1)'. From there you can find more
;;	detailed information of incoming message queue, than the simple
;;	message count in echo-area or x-drag bar.
;;
...
;;
;;	_Note_: VM and Gnus can keep the sent mail buffer around. This package won't
;;	install `tinymail-save-dead-mail-maybe' the dead mail collector under
;;      Gnus and VM.
;;
...
;;	an individual who take measures to protect himself from getting
;;	into the email harvester's "2 billion email address on a CD for ;;
;;	$100"
;;
;;	Here is an opinion whether it is right to munge the
;;	address according to RFC hear by 1997-11-05 gnus.emacs.gnus,
;;      Marty Fouts:
;;
...
;;	o   Not all people have access to filtering tools (e.g. Unix Procmail).
...
;;          first place.
;;
;;	This package can activate the address munging very easily
;;	for selected newsgroups and make those email harverters gathering job
;;	more difficult. Humans that want to contact you can still decode the
;;	address if they want to talk to you directly.
;;
;;	To activate address munging for newsgroups matching regexp, set variable
;;	`tinymail-:from-anti-ube-regexp'. Your `user-mail-address' is be hashed
;;	and different address is generated every time.
;;
...
;;	  during transit. They may not appear in the exact place where
...
;;	and when messages are returned to you, you can file the messages to proper
;;	folders. For me the  my procmail traps the plus information and files,
...
;;	by function `tinymail-message-disable-sender', so that From field is
...
;;	information to use bt setting function to `tinymail-:from-info-function'.
;;	Non-Group posting means, that you're not inside ant Gnus Newsgroup
;;	from where you initiate "post". A typical invocation to no-Group
;;	posting is `C-x' `m'.
;;
;;     Newsgroup posting
;;
;;	You might want to set `tinymail-:from-info-function' return
;;	different email address for Usenet newsgroup posts. Order some free
;;	email and use that for Usenet discussions.  That way you can
;;	reserve your normal address to your private email communication.
;;
;;	The settings you need to enable the address generation is quite
;;	basic. Table `tinymail-:from-table-prefix' sets the left part of the
;;	plus address component and `tinymail-:from-table-postfix' can set the
...
;;	`user-full-name' part. If `tinymail-:from-table-prefix' returns
;;	nothing, the `tinymail-:from-table-postfix' is used as is.  Here is
;;	example setup. Pay attention to the "work.misc" which is sole return
;;	value for all addresses matching "my-work-site".
;;
;;          (defconst tinymail-:from-table-prefix
;;            '(("emacs\\|perl" . "mail")
;;          	("."   "usenet"))
;;          ;;
;;
;;          (defconst tinymail-:from-table-postfix
;;            '(("games"				 . "games")
...
;;     Gnus support
;;
;;	If you use Gnus as a news reader, then you get some bonus. There are lot
;;	added value for gnus users so that the default plus information is
;;	generated based on the group you're posting from. In general the
...
;;
;;	In Win32 you usually swap between on-line and off-line. If you use
;;	Gnus as your mail reader, TinyMail can show the plugged status
;;	in the mode string. If you see "+" you're plugged (on-line). The key
;;	to change the Gnus plugged status is bound to `C-c' `t' `j'
;;
...
;;
;;	This feature is mainly designed for Gnus `message-mode'. Use it like:
;;
;;	    (setq mail-yank-prefix  "| ")
;;	    (setq mail-user-agent   'message-user-agent)
;;
;;	In this package you find function `tinymail-citation-generate' which
...
;;
;;	and have it return `t' if your function did something. See
;;	also `tinymail-:table-header-complete' where you can define
;;	custom headers and the completions easily.
;;
;;	[2000-09] Tha tab key no longer calls separate functions that
;;	collect the information one by one, but calls a single function
;,	which gathers all possible email completions and then presents the
;;	list via completion to user. The function that should
;;	be first is `tinymail-complete-everything'. If you're previous
;;	user and find this "all in one list" disturbing and you would like
;;	to continue using the older behavior "try this, try next.." which
;;	gives better separation of what is being completed (mailalias,
;;	passwords, bbdb etc..), put the function in hook in this order:
;;
;;
;;	    tinymail-complete-bbdb-fuzzy	;honor BBDB first
;;	    tinymail-complete-guess-in-headers  ;then passwd
;;	    tinymail-complete-simple		;then rest of the fields
;;	    tinymail-complete-headers-nothing-found
;;	    tinymail-complete-abbrevs
;;	    tinymail-complete-guest-packages
;;
;;	    ;; And your own custom complete functions after this
;;
...
(eval-and-compile
  ...
  (if (null (locate-library "bbdb"))
      (message "\
  ** tinymail.el: No bbdb.el along load-path. Please do not compile this file.
                  http://bbdb.sourceforge.net/
                  You can still use the package if you DO NOT BYTE COMPILE FILE.
                  Package will adapt to missing BBDB."))
...
(ti::package-defgroup-tiny TinyMail tinymail-: mail
  "Some mail additions: dynamic Fcc, Cc, hilit, X-Sender-Info.
	Overview of features

	o  Some handy additions to mail sending interface.
	o  Adds automatically Cc field when you type the To: address.
	o  Adds X-Sender-Info dynamically while you type To: address.
	o  Changes Fcc dynamically according to header content.
	o  Very easy TAB completion: two modes, alias and definition string.
...
;; Without fully qualified domain name, in the Win32 SMTP smtpmail.el
;; can't send messages. Make sure the email is format user@domain.com

(unless (string-match ".+@.*\\..+"
		      (or user-mail-address
			  ""))
  (error "Tinymail: Please set `user-mail-address' to \"user@somewhere.net\""))
...
Group parameter for each group. This hook contains default function
`tinymail-mail-send-to-list' which send your amil to mailing list when you're
using such group that has `to-list'"
  :type 'hook
  :group 'TinyMail)


(defcustom tinymail-:complete-key-hook nil
...
(defcustom tinymail-:table-keymap-list
  '(mail-mode-map
    message-mode-map
    mh-letter-mode-map)
  "*List of keymaps where to install default bindings. (TIMI pacakge)."
...
(defcustom tinymail-:cc-kill-regexp (regexp-quote user-mail-address)
...
(defcustom tinymail-:x-info-label "X-Sender-Info"
  "*Extra info field label. There must be NO COLON or SPACES.
User addd header names must start with X-* according to RFC."
  :type '(string :tag "Header name")
  :group 'TinyMail)
...
(defcustom tinymail-:password-cat-cmd
  (cond
   ((string-match "hpux\\|hppa" (emacs-version))
    "ypcat passwd")
   ((string-match "solaris" (emacs-version))
    "cat /etc/passwd")
   ((string-match "sparc\\|sun\\|sunos" (emacs-version))
    "cat /etc/passwd")
   ((string-match "sgi-irix\\|irix" (emacs-version))
    "ypcat passwd")
   ...
   (t
    (error "TinyMail: tinymail-:password-cat-cmd, Please contact maintainer.")))
...
(defcustom tinymail-:table-x-info nil
  "*The X-Sender-Info: field is added if REGEXP entry match To: field.
The LISP-FORM can be string or any form that yields string.
Remember that according to RFC the continuing lines must have
\\t or space at the beginning. E.g.

X-Sender-Info: line-1-comes-here      [newline]
\[ tab  ]line-2-continues-here  [newline]
\[ tab  ]line-3-continues-here  [newline]


Format is '((REGEXP . LISP-FORM) (REGEXP . LISP-FORM) ..)"
  :type  '(repeat
	   (cons
	    (string :tag "Regexp")
	    (sexp   :tag "Form")))
...
the address. The generated email is made with `tinymail-from-anti-ube-address'.
...
Notes
    ...
    can use function `tinymail-from-anti-ube-address' which uses hash table
    ...
    (defun my-tinymail-address ()
      ...
      (cond
        ((string-match \"games\\\\|ibm\" group)
         (list (tinymail-from-anti-ube-address addr)))   ;; grumbled address
	((string= \"\" group)
...
(defcustom tinymail-:report-window-system (ti::xe-window-system)
    ...
    (let ((mail (getenv "MAIL"))
      ...
      (setq
       ...
       (or (and (file-exists-p "~/.procmailrc")  ;; [1]
		(error "\
TinyMail: autosetup aborted. You seem to use Procmail. Please set
`tinymail-:report-mail-notify-program' to cover possible
multiple folders."))
	   ...
	   (and mail    	       ;; [3]
		(or (file-exists-p mail)
		    ...
		    (error "TinyMail: Environment variable MAIL is invalid: %s "
			   mail))
	   ...
	   (let ((function (if (win32-p)
			       'message
			     'error)))
	     (funcall function "\				    ;; [4]
TinyMail: Can't guess `tinymail-:report-mail-notify-program'. Set manually.")
	     nil)))
      (when cmd
	(cond
	 ((not (win32-p))			;Unix, return as is
	  cmd)
	 ((and (win32-p)
	       (string-match "bin" (or shell-file-name "")))
	  ;; This system is running Cygwin bash
...
(defcustom tinymail-:report-keep-intact-list
  '("VM")
  "*A list with frame names not to change."
  :type '(repeat string)
  :group 'TinyMail)
...
the echo area if Emacs is running in non window envinronment."
  :type 'string
  :group 'TinyMail)
...
(defcustom tinymail-:report-mail-info-shorten-regexp nil
  "*Regexp to matc local site address.
...
(defvar tinymail-:face1 'highlight
  "Mail face. 'Subject'.")

(defvar tinymail-:face2 'bold-italic
  "Mail face. 'Fcc' or 'Gcc'.")

(defvar tinymail-:face3 'highlight
  "Mail face. 'Cc' or 'Newsgroup'.")

(defvar tinymail-:face4
  (or (and (ti::xe-face-p 'region)			;XEmacs 19.14 doesn't have this
	   'region)
      'bold)
  "Mail face. 'To' field.")
...
(ti::macrof-debug-standard "tinymail" "-:")
...
(defun tinymail-install-hooks (&optional remove verb)
  ...
  (let* (
	 ;; The first function run must be last in this list.
	 (list '(
		 tinymail-complete-guest-packages
		 tinymail-complete-abbrevs
		 tinymail-complete-headers-nothing-found
		 ...
		 ;;  BBDB record. We use fuzzy instead
    ...
    (cond
     ...
     ((null tinymail-:complete-key-hook)
      ...
      (ti::add-hooks 'tinymail-:complete-key-hook list)))
    ...
    (when (memq 'message-header-setup-hook  tinymail-:enter-mail-hook-list)
      (ti::add-hooks 'message-mode-hook 'tinymail-message-disable-sender remove))
...
(defun tinymail-install-table-header-complete-gnus ()
  ...
  (aput 'tinymail-:table-header-complete
	...
	'("critical"	;; Whole system break, serious data loss
	  ...
	  "important"	;; major effect withour  completely unusable to everyone.
  ...
  (aput 'tinymail-:table-header-complete
	...
	'("patch"	;;
	  ...
	  "sid"))	;;  architecture that is currently unreleased
  ...
  (aput 'tinymail-:table-header-complete
	"Newsgroups"
	'(if (not (featurep 'gnus))
	     (prog1 nil (message "TinyMail: Newsgroups completion needs Gnus..."))
	   (when (stringp string))
	   (all-completions
	    string
	    gnus-active-hashtb
...
(defun tinymail-read-version (sym)
  "Read version number from variable SYM if variable exists. Otherwise return ''."
...
;;; ----------------------------------------------------------------------
;;;
(defun tinymail-install-default-x-info  ()
  "Install defualt entries into `tinymail-:table-x-info'."
  (interactive)
  (let* ()

    ;;  send 'tiny tool' library information when talking
    ;;  with the maintainer.

    (aput
     'tinymail-:table-x-info
     "jaalto\\|ssjaaa\\|jari.aalto"
      '(format (concat "E%s lib%s b%s m%s mt%s xe%s o%s ma%s pg%s \n"
		      "  mi%s my%s di%s dd%s tf%s li%s tit%s %s ")
	      (tinymail-read-version 'emacs-version)

	      ;;  Libraries

	      (tinymail-read-version 'tinylib-version)
	      (tinymail-read-version 'tinylibb-version)
	      (tinymail-read-version 'tinylibm-version)
	      (tinymail-read-version 'tinylibmail-version)
	      (tinymail-read-version 'tinylibxe-version)
	      (tinymail-read-version 'tinylibo-version)
	      (tinymail-read-version 'ti::text-:version-id)

	      ;; programs

	      (tinymail-read-version 'tipgp-:version-id)
	      (tinymail-read-version 'tinymail-:version-id)

	      (tinymail-read-version 'timy-:version-id)
	      (tinymail-read-version 'tdi-:version-id)
	      (tinymail-read-version 'tdd-:version-id)
	      (tinymail-read-version 'titf-:version-id)
	      (tinymail-read-version 'tili-:version-id)
	      (tinymail-read-version 'tinytab-:version-id)

	      ;;  Tell also what mail agent user uses.
	      ;;  If you're not in this list, contact jari!

	      (cond
	       ((get-buffer "RMAIL") "RM")
	       ((boundp 'vm-version)   (concat "VM" vm-version))
	       ((boundp 'mh-e-version) (concat "MH" h-e-version))
	       ((boundp 'gnus-version) gnus-version)
	       (t
		"?"))))))
...
TinyMail: This Emacs does not support idle timers. Using regular timers."))
    ...
    (ti::xe-timer-cancel-function 'tinymail-process)
	...
	;;  so use 6 seconds repeat time.
	(setq tinymail-:timer-elt
	      (run-at-time "1 sec" 6 'tinymail-process)))

      (tinymail-install-default-x-info)
      (tinymail-install-hooks remove)

      (when (ti::nil-p (user-full-name))
	(error "TinyMail: please set variable `user-full-name'.\n"))

      (tinymail-install-table-header-complete-gnus)
      (tinymail-install-to-buffers)
      (message
       "TinyMail: Installed. M-x tinymail-version for documentation."))))

;;; ----------------------------------------------------------------------
;;;
(defun tinymail-install-citation ()
  "Install First line citation function for Mail user agents."
  ;; Use defconst because this variable is not defined when we byte compile
  ;; this file and (require 'xxxx) would be waste of time.
  (defconst message-citation-line-function
...
Return:
  ...
  (let ((limit (- 75 13))               ; line-lenght - date  ==> limit
    ...
    (when ret
      ...
      (setq ret (replace-regexps-in-string "['\"]+" "" ret))
      ;;  Remove Middle names: Foo X. Bar
      (setq ret (replace-regexps-in-string " [A-Z]\\." "" ret)))
...
- Private mail does _not_ include any extra references."
  (let* ((id (mail-fetch-field "References"))
    ...
    (and id
	 ...
	 (setq id  (car (reverse (split-string id))))
	 (setq url (ti::string-match "<\\([^ \t\n>]+\\)>" 1 id))
	 (setq url
	       (concat
		...
		"%3E&format=threaded>")))
    (cond
     ((ti::mail-news-buffer-p)
      ;;  In Newsgroups it is better to use URL references
      (concat "* " url "\n"))
     ((ti::mail-to-list-p)
...
possible to retrieve whole News thread e.g. from dejanews. Message-Id is
a handly way to refer to past articles."
  (let* ((hdrs (if (boundp 'message-reply-headers)
    ...
    (insert "* " date " " from "\n"
	    (funcall tinymail-:citation-message-id-function))))
...
If there is 2 or more leading spaces, then the field is considered 'off'."
  (when (stringp header-name)
    (not (memq (ti::mail-field-space-count header-name header-value) '(0 1)))))
...
(defun tinymail-make-local-hook-available-p ()
  ...
  (or (xemacs-p)			;Always in XEmacs
      (and (emacs-p)		;19.30 and up
...
(defsubst tinymail-y-or-n-p-abort-p ()
  ...
  (char-in-list-case tinymail-:y-or-n-p '(?q ?Q)))
  ...
  (setq tinymail-:y-or-n-p
	(ti::read-char-safe-until
	 ...
	 '(?\t
	   ...
	   ?\~)))
  ;;  YES answers.
  (char-in-list-case tinymail-:y-or-n-p '(?y ?Y ?\ ?\t ?\n ?\r)))
...
documentation in the tinymail.el or call \\[tinymail-version]."
  ...
  (if (ti::nil-p user-mail-address)
    ...
    (if (interactive-p)
        (message "Address changed to point to you. TIMI signs off."))))

;;; ----------------------------------------------------------------------
;;;
(defun tinymail-active-p ()
  "Check if timi mail is on in current buffer."
  tinymail-mode)
...
References:
  ...
  (let* ((fid "tinymail-mail")
    ...
    (when (featurep 'tinytab)
      ...
      ;;    runs first.
      (remove-hook 'tinytab-:tab-insert-hook 'tinymail-complete-key)
      (add-hook    'tinytab-:tab-insert-hook 'tinymail-complete-key))

    (unless disable

      ;;  If you're replying to someone else's message, the TO field
      ;;  must have two spaces to turn off TIMI so that the remaining
      ;;  Cc fields are not modified. (VM's F key)
      ;;
...
;;; ----------------------------------------------------------------------
;;; - This is just an example function, Demonstrates how you can send
;;;   the version information automatically, See tinymail-:table-x-info
;;;
(defun tinymail-mail-info-ange ()
  "Return ange-ftp/efs info."
  (if (boundp 'efs-19-version)
      (concat "efs "
              ;;  This is only needed to shut up the 19.28 Byte Compiler
              ;;  Which can't see the bounndp test...
              (symbol-value 'efs-19-version)
              "1.14\n")
    (concat "ange-ftp," emacs-version)))
...
in *Group* buffer):
  ...
  (when (eq major-mode 'message-mode)
    (let* ((fid   "tinymail-mail-send-to-list:")
	   ...
	   (email (if to (car-safe (ti::mail-email-from-string to)))))
	...
	(tinymail-debug fid "SELECTION" mode str elt)
      ...
      (cond
       ((and verb (null done) str)
        (message (format "Timi: no completion match on '%s'" str)))
...
(defun tinymail-complete-simple (&optional info)
  "Complete headers according to `tinymail-:table-header-complete'.
INFO is '(string beg end) of the completion word"
  (interactive)
  (ti::mail-point-in-header-macro
    (let* ((fid        "tinymail-complete-simple: ")
	   (field      (ti::remove-properties (ti::mail-current-field-name)))
	   (field-info (or info (tinymail-complete-string-read)))
	   multi-word
	   complete-list
	   tmp
	   choices
      ...
      ;;  The STRING is dymamically bound and visible for EVAL CHOICES

      (when (stringp (setq string (nth 2 field-info)))
	(setq choices
	      (if field
		  (cdr (assoc field tinymail-:table-header-complete)))))
      ...
      (when choices
	(if (not (stringp (car choices)))
	    (setq choices (eval choices))))

      (tinymail-debug fid "CHOICES" choices)
...
(defun tinymail-complete-everything (&optional verb)
  ...
  (let ((data (tinymail-complete-string-read)))
	...
	(if (and verb (null done) choice)
	    (message (format "Timi: no completion match on '%s'" choice)))
...
functions. Each function is passed the word info at point: '(BEG END STRING)."
  ...
  (when (ti::mail-mail-p)
    (tinymail-debug 'tinymail-complete-key 'HEADER-CHECK header-check
		    (save-excursion
		      (concat "\n************ START **********\n"
			      (buffer-substring
	...
	(if (not (string-match "tinymail"
      ...
      (setq tinytab-:tab-insert-hook (reverse clean-hook))
	...
	(when (and header-check
		   (not (ti::mail-field-email-send-p
			 nil ;; read current field name where point is
...
;;; ----------------------------------------------------------------------
;;; In gnus 5 you can say: (setq sage-kill-buffer-on-exit 'yes)
;;;
(defadvice mail-send-and-exit (after timi dis)        ;; Disabled 1999-11-17
  "Kill mail buffer after send."
  (let* (buffer)
    ;;  There is bury buffer command... get that back
    (setq buffer (car (reverse (buffer-list))))

    (if (string-match "mail" (buffer-name buffer)) ;be cautious
        (kill-buffer buffer))))

;;  Old message has autosave name "*message*", but that does not work in
;;  Win32 platform (C-x m  M-x message-modew and  C-x s and Emacs dies on error)
;;
;;  The advice code corrects it to #message#
...
(defun tinymail-save-dead-mail ()
  "Loop mail buffers and save them to `tinymail-:dead-mail-file' on Emacs exit."
  (ti::dolist-buffer-list
   ...
   (progn
     ...
     (append-to-file (point-min) (point-max) tinymail-:dead-mail-file))))
  ...
  (let* ((buffer  (and (stringp tinymail-:report-spool-buffer)
    ...
    (when buffer
	  ...
	  (setq tinymail-:report-old-frame-string
		(ti::xe-set-frame-name nil nil 'get)))
	...
	(dolist (elt (frame-list))	;Update frames that are not exluded
	  (if (not (member
		    (ti::xe-set-frame-name nil nil 'get)
		    tinymail-:report-keep-intact-list))
	      (ti::xe-set-frame-name display-string elt))))
...
TinyMail: tinymail-report-mail-install: 'reportmail feature found, install ignored.")
    ...
    (when (and tinymail-:report-window-system (xemacs-p))
      ;;  make sure it's list
    ...
    ;; Delete old timer
    (ti::xe-timer-cancel-function 'tinymail-report-update)
    (setq tinymail-:report-timer-object nil)

    (unless uninstall
      (setq tinymail-:report-timer-object
	    (run-at-time "1 min" 60 'tinymail-report-update)))
...
;;; ----------------------------------------------------------------------
;;;
(defun tinymail-report-mail-install-maybe (&optional verb)
...
E.g. In XEmacs you can use package reportmail.el."
  ...
  (cond
   ...
   ((featurep 'reportmail)
    (message "TinyMail: reportmail present, not installing own reportmail."))
   (t
    (tinymail-report-mail-install nil verb))))
...
;;; ----------------------------------------------------------------------
;;;
(defun tinymail-from-anti-ube-address  (email)
  "Make Simple anti-spam address."
  (let* ((base [ "uce" "ube" "spam" "commercials"
	         "advertisements" "ads" "junk" "garbage"
	       ])
	 (add  [ "no" "hate" "stop" "die" "hang" "fool" "yuck"
		 "dislike" "go-away" "stay-away" "delete" "erase"
		 "zap" "wipe" "exterminate"
		 "kill" "ignore" "bypass" "keep-out" "keep-away"
		 "none" "nada" "zero" "not-any" "zelt"
		 "no-thank-you"
		 "remove-this" "rip-off-this" "disregard"
		 "throw-away"
	       ])
	 (vec  (vector
		(concat
...
(defun tinymail-from-anti-ube-maybe ()
  ...
  (when (and (stringp tinymail-:from-anti-ube-regexp)
    ...
    (let* ((group (mail-fetch-field "Newsgroups")))
      (when (string-match tinymail-:from-anti-ube-regexp (or group ""))
	(tinymail-from-anti-ube-address user-mail-address)))))
...
(defun tinymail-from-field-value-plus ()
  ...
  (let* ((fid	   "tinymail-from-field-value-plus:")
	 ...
	 ;;  Pposting from Gnus, so get the Group name
	 ;;
    ...
    (cond
     ...
     (group
      ;; If I'm posting from inside Group, add Group based PLUS address
      ;;
      ;;  list.xxx --> '("list" "xxx")
...
(defun tinymail-message-disable-sender ()
  "Disable Sender field generation permanently. You want this in Win32."
  ;; Gnus message-mode
...
;;; ----------------------------------------------------------------------
;;;
(defun tinymail-field-x-info (&optional to-field hsize)
  "Add info field to message.

Input:

 TO-FIELD   To field content. If nil then read from message.
	    This can also be 'newgroups' field content.
 HSIZE	    Header area size. If nil then read from message."
  (let* ((fid       "tinymail-field-x-info: ")
	 (label     tinymail-:x-info-label)
         (table     tinymail-:table-x-info)
         (hsize-old (get 'tinymail-:table-x-info 'old-hsize))
         info evaled)
    (or to-field
	(or
	 (setq to-field (ti::mail-get-field-1 "To"))
	 (setq to-field (ti::mail-get-field-1 "newsgroups"))))

    (or hsize
	(setq hsize (ti::mail-header-area-size)))

    (tinymail-debug fid "in: to-field" to-field "HSIZE" hsize "OLD" hsize-old)

    ;;  anything changed since the last call?

    (unless (eq hsize hsize-old)
      (cond
       ((and (not (ti::nil-p to-field))
	     (setq info (ti::list-find table (ti::string-remove-whitespace to-field))))
	(setq evaled (eval (cdr info)))
	(tinymail-debug fid evaled info)
	(when (stringp evaled)
	  (ti::mail-add-field label evaled "Subject" nil 'replace)))
       (t
	(ti::mail-kill-field label))))

    (put 'tinymail-:table-x-info 'old-hsize hsize)))
...
(defun tinymail-field-to-on ()
  "Keep TIMI activated by making sure the To: field has only one space."
...
(defun tinymail-field-to-off (&optional count field)
  "Disable TIMI by space COUNT for FIELD.
2  spaces disables Cc tracking, but allows X-Sender-Info tracking.
3  spaces disables both Cc and X-Sender-Info tracking."
  (let* (str)
...
(defun tinymail-from-set-field (&optional from-field)
  ...
  (let ((fid "tinymail-from-set-field:")
	str)

    (or from-field
	(setq from-field (ti::mail-get-field-1 "From")))
    ...
    (save-excursion
      (cond
       ((null from-field)
	(setq str (tinymail-from-field-value))
	(ti::pmin)
	(tinymail-debug fid 'from-generated str)
	(insert "From: " str "\n"))

       ((not (string-match "^  " from-field))
	...
	(ti::mail-kill-field "From" (tinymail-from-field-value)))))))
...
C-u M-x tinymail-process-1."
  ...
  (let* ((fid           "tinymail-process: ")
    ...
    (cond
     ...
     ((and (tinymail-field-off-p nil to) ;Only X-Sender-Info tracking
	   (not (ti::nil-p to))		;Not just spaces ?
	   (not (string= to last-to)))
      (tinymail-field-x-info to hsize))
	...
	(ti::mail-abbrev-expand-mail-aliases
	 (point-min) (ti::mail-hmax) alias-alist))
      (setq to (ti::mail-get-field "To")) ;what was expanded
      (tinymail-field-x-info to)))
...
(tinymail-install)
(tinymail-report-mail-install-maybe)
