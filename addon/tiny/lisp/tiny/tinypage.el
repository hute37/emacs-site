...
;;; @(#) $Id: tinypage.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:	    tools
;;
;; To get information on this program use ident(1) or do M-x tinypage-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinypage.el|Jari Aalto|jari.aalto@poboxes.com|
;; ^L page select, cut, copy. Renumbering 1.1 etc. levels. No narrowing used |
;; 2002-01-27|$Revision: 1.1 $|~/misc/tinypage.el.Z|
...
(defun tinypage-toc-x-popup-keyboard ()
  ...
  (tinypage-toc-x-popup
   (ti::xe-make-fake-event tinypage-:x-coord tinypage-:y-coord)))
  ...
  (let* ((len    tinypage-:x-popup-line-len)
    ...
    (cond
     ...
     (t
      ...
      (when (setq val (ti::xe-popup list event nil title))
