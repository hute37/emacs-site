...
;;; @(#) $Id: tinydiff.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Author:          Jari Aalto <jari.aalto@poboxes.c1om>
;; Maintainer:	    Jari Aalto <jari.aalto@poboxes.com>
;; Created:	    1996-01
;; Keywords:	    tools
;;
;; To get information on this program use ident(1) or do M-x tinydiff-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinydiff|Jari Aalto|jari.aalto@poboxes.com|
;; Diff minor mode. Diff browsing, applying email patches easily.|
;; 2002-08-01|$Revision: 1.1 $|~/misc/tinydiff.el.Z|
...
;;   ~/.emacs startup file
;;   Code can be easily extracted with C-x h  +  M-x ti::package-rip-magic
;;
...
;;
;;* _   (ti::use-prefix-key global-map "\C-cmd")  ;; Clears key "\C-cmd"
...
;;
;;      M-x tinydiff-debug-toggle            turn on debug before you send bug rpt
...
(eval-and-compile
  ...
  (autoload 'tinymy-read-only     "tinymy" "" t)
...
(defcustom tinydiff-:mode-define-keys-minibuffer-hook
  ...
  "*Function to define the keys for the mini buffer."
  :type  'hook
  :group 'TinyDiff)
...
Input:
  ...
  SEEK-OPTION	    When GNU product, it must know this option."
	...
	(dolist (prg program-list)
	  (message "TinyDiff: Please wait. Examining %s" prg)
...
(defcustom tinydiff-:diff-program
  (cond
   ((boundp 'diff-command)
    diff-command)
   ((boundp 'ediff-diff-program)
    ediff-diff-program)
   (t
    (tinydiff-find-program '("gdiff" "diff") "diff" nil "--help")))
...
(defcustom tinydiff-:patch-program
  (tinydiff-find-program '("gpatch" "patch") "patch" "-t -N -F 3" "--help")
...
(defvar tinydiff-:last-data nil
  "Private. Data storage eg in `tinydiff-cl-change-diff-command'.")
...
(defvar tinydiff-:minibuffer-map nil
  "Mini buffer key map when asked for the right diff command.")
...
(defun tinydiff-mode-define-keys-minibuffer-default ()
  "This function defines some extra bindings to mini buffer.
Eg. TAB that completes current filename."
  ...
  (define-key tinydiff-:minibuffer-map "\t"       'tinydiff-cl-complete-filename)
  (define-key tinydiff-:minibuffer-map [(kp-tab)] 'tinydiff-cl-complete-filename)
  ...
  (define-key tinydiff-:minibuffer-map "\C-z" 'tinydiff-cl-change-diff-command)
  (define-key tinydiff-:minibuffer-map "\C-r" 'tinydiff-cl-rev-add-command)
  (define-key tinydiff-:minibuffer-map "\C-c" 'tinydiff-cl-insert-file-autosave)
  (define-key tinydiff-:minibuffer-map "\C-v" 'tinydiff-cl-insert-file-backup)
  (define-key tinydiff-:minibuffer-map "\C-s" 'tinydiff-cl-toggle-diff-type)
  (define-key tinydiff-:minibuffer-map "\C-f" 'tinydiff-cl-user-option)
  (define-key tinydiff-:minibuffer-map "\C-p" 'tinydiff-cl-insert-previous-word)

  (define-key tinydiff-:minibuffer-map "?" 'tinydiff-cl-minibuffer-help))
...
(ti::macrof-debug-standard "tinydiff" "-:")
...
The passed CMD must always be in the format:

   cd DIRECTORY; BINARY option option option ...."
  (let* ((info               (tinydiff-cl-splice-command cmd))
	 (default-directory  (or (car info)
				 (error "TinyDiff: No DIR in %s" cmd)))
	 (cmd                (nth 1 info)))
    ;;  tinydiff-cl-splice-command has failed if this happens.
    (unless (file-directory-p default-directory)
      (error "TinyDiff: default-directory parse error. Please check cmd-line"))
    (setq buffer (get-buffer-create buffer))

    ;; win32 does not know ~ expansions
    (setq cmd (ti::expand-file-name-tilde-in-string cmd))

    ;; (call-process cmd nil buffer nil arg arg)

    ;;  #todo, should use call-process
    (shell-command cmd buffer)

    (when (or (null buffer)
	      (null (get-buffer buffer))
	      (not (buffer-live-p buffer)))
      (error "TinyDiff: Shell dind't return results for command [ %s ]" cmd))))

;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-update-revision-list (file &optional version)
  "Read all revision numbers for FILE starting from VERSION."
  (let* ((ver   (or version "1.1"))
         (fid   "tinydiff-update-revision-list: ")
         (dots  (count-char-in-string ?. ver)))
    (when
      (null tinydiff-:version-list)
...
Switches to BUFFER and go to LINE and calls `beginning-of-defun'"
  (let* ((func          tinydiff-:function-name-handle-function)
    ...
    (with-current-buffer buffer
      (ti::widen-safe
	(goto-line line)
;;;       (ti::d! (current-buffer) (ti::current-line-number) line (ti::read-current-line))
...
(defun tinydiff-turn-on-view-mode ()
  "Turn on view mode.
Call `tinymy-read-only' to turn on the appropriate view mode. also
detect if you're using tinyezip.el

If there is no tinymy.el available, turn on simple function `view-mode'."
  ;;  We have to load the full package if tinyezip is used.
  ;;
  (cond
   ((null tinydiff-:package-exist-tinymy)
    (view-mode 1))
   (t
    (if (featurep 'tinyezip)
        (require 'tinymy)
      ;;  This allow dynamic byte loading, only this function
      ;;  is loaded if you have byte compiled the package.
      (autoload 'tinymy-read-only "tinymy"))
    (with-current-buffer tinydiff-:diff-buffer
      (ti::pmin)
      (tinymy-read-only)))))
...
;;; cl = command line
;;;
(defun tinydiff-cl-replace-text (beg end text)
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-read-revision ()
...
Return:
 ...
 nil"
  (interactive)
  (let* ((fid "tinydiff-cl-read-revision: ")
         nbr
         pos)
    (save-excursion
...
;; cd /users/jaalto/elisp/; rcsdiff -c -r1.25 -r1.8 tinydiff.el
      (cond
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-splice-command  (string)
  "Splice off directory from string and return '(DIR REST)."
  (when (string-match "cd[ \t]+\\([^;]+\\);[ \t]*\\(.*\\)" string)
    (list (match-string 1 string) (match-string 2 string))))

;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-read-rcs-file-name (&optional line or-diff-name)
  "Read RCS filename from LINE.
If OR-DIFF-NAME is non-nil, look for 'diff' command instead."
  (let* ((line      (or line (ti::read-current-line)))
         (rcsdiff   tinydiff-:rcsdiff-program)
         (fid       "tinydiff-cl-read-rcs-file-name: ")
...
;;;
(defun tinydiff-cl-rev-add-command  ()
...
-->     Do nothing, since there is already two -r switches."
  (interactive)
  (let* ((fid       "tinydiff-cl-rev-add-command: ")
	...
	(when (string-match "\\.\\([0-9]+\\)$" tmp)
	  (setq revision (string-to-int (match-string 1 tmp))
		nbr      (string-to-int (ti::string-right (match-string 1 tmp) 1)))
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-toggle-diff-type  ()
  "Toggle -c context or -u unified diff option in command line.
the option must surrounded by spaces: -cbw won't be changed."
  (interactive)
  (let* ((line	    (ti::remove-properties (ti::read-current-line)))
	 ret)
    (cond
     ((string-match " -c " line) (setq ret (ti::replace-match 0 " -u " line)))
     ((string-match " -u " line) (setq ret (ti::replace-match 0 " -c " line))))
    (when ret
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-change-diff-command  ()
  "Change diff command in mini buffer."
  (interactive)
  (let* ((fid      "tinydiff-cl-change-diff-command: ")
	 (rcsdiff  tinydiff-:rcsdiff-program)
         (diff     tinydiff-:diff-program)
         word
         line
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-sweep-unix () ;; win32
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-complete-filename ()
  "Complete filename."
  (interactive)
  (tinydiff-cl-sweep-unix)
  (let* ((fid	"tinydiff-cl-complete-filename")
	 (word  (save-excursion
                  (forward-char -1)
                 (ti::buffer-read-space-word)))
         (dir   default-directory))
    (tinydiff-debug fid word dir)
    (unless (ti::nil-p word)
      ;;   The directory name we get from the 'CD' prompt if the
      ;;   filename lacks dir part.
      (if (not (string-match "/" word))
          (setq dir (tinydiff-cl-directory)))
      (let ((default-directory (or dir default-directory)))
	(ti::file-complete-file-name-word)))))

;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-insert-file-autosave  (&optional backup)
...
Prefix arg says to insert BACKUP filename instead."
  ...
  (let* ((line      (ti::read-current-line))
         (rcsdiff   tinydiff-:rcsdiff-program)
         (fid       "tinydiff-cl-insert-file-autosave: ") ;function id
         file
         file2
         stat)

    (setq file (tinydiff-cl-read-rcs-file-name line 'diff))
      ...
      (message
       (substitute-command-keys
        (concat
         ...
         "\\[tinydiff-cl-change-diff-command] "))))
     ((not file)
...
;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-insert-file-backup  ()
  "Command line. Inset backup filename if it exists."
  (interactive)
  (tinydiff-cl-insert-file-autosave 'backup))

;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-minibuffer-help ()
  "Show brief help."
  (interactive)
  ...
  (with-current-buffer "*Help*"
    (ti::pmin)
    (forward-line 4)
    (delete-non-matching-lines "tinydiff-cl")))

;;; ----------------------------------------------------------------------
;;;
(defun tinydiff-cl-insert-previous-word ()
...
Input:
  ...
  (let* ((fid		"tinydiff-patch-with-diff-1")
    ...
    (setq ff  (file-name-nondirectory file))

    (setq buffer        (ti::temp-buffer tinydiff-:diff-tmp-buffer 'clear))
    (setq data-buffer   (ti::temp-buffer tinydiff-:patch-tmp-buffer 'clear))

    (with-current-buffer data-buffer
      (insert-buffer-substring source-buffer beg end)
      (tinydiff-patch-cleanup)

;;;      (pop-to-buffer (current-buffer))      (ti::d! "ok")

      ;;  Sometimes user's UMASK is not ok. BE sure that the file is
      ;;  readable after the write.
      ;;
      (if (not (file-writable-p diff-tmp))
          ;;   Make it writable then
          (ti::file-chmod-w-toggle diff-tmp))

      (ti::write-file-as-is-macro
	(write-region (point-min) (point-max) diff-tmp))

      (if (not (file-readable-p diff-tmp))
          (set-file-modes diff-tmp 384))       ;; -rw-------

      (setq cmd (concat "cd " dir "; " prg " " ff " " diff-tmp))
...
;;; ----------------------------------------------------------- &patch ---
;;;
(defun tinydiff-patch (arg &optional beg end dest-file verb type)
...
;;;    (ti::d! beg end)
    ...
    (unless (stringp dest-file)
      ...
      ;;  Story of this starange WRAP (save-ex, buffer change ..)
      ;;  - I was receiving diff in RMAIL buffer. When I called this function
      ;;    read-file-name from RMAIL buffer it always gave me RMAIL,
      ;;    not the DIR/FILE combination.
      ;;  - I have no idea why the read-file-name went beserk, but changing
      ;;    the buffer to some *temp* cured the problem
      ;;  - This is highly dangerous, since before I noticed this
      ;;    the DEST-FILE was set to RMAIL, and the patch was done against
      ;;    my mail buffer!  (Hopefully I had it in Emacs, so I could wrote
      ;;    the original back...)

      (save-excursion
        (set-buffer buffer)
        (setq dest-file (read-file-name "Apply diff to: " nil nil t))))

    (if (or (not (stringp dest-file))
            (not (file-exists-p dest-file)))
        (error "TinyDiff: Cannot patch non-existing file. Cancelled."))
...
(defun tinydiff-diff-command-generate  (&optional no-ask)
  ...
  (let* ((map           tinydiff-:minibuffer-map)
         (tmp-file      (if (win32-shell-p)
			    (expand-file-name
			     tinydiff-:diff-tmp-file)
			  tinydiff-:diff-tmp-file))
         (diff-prg      (eval tinydiff-:diff-program))
    ...
    (when bf
      ...
      (setq file (file-name-nondirectory bf)))
    ...
    (setq prompt (format  "cd %s; %s %s %s %s "
                          dir
                          diff-prg options
                          file
                          file2))
    ...
    (if no-ask
        (setq ans prompt)
      (let* (tief-mode)		;Electric file mode OFF
	(if tief-mode
...
Mark diff lines for special handling."
  ...
  (let* ((diff-type      (car-safe (ti::buffer-diff-type-p)))
    ...
    (ti::text-clear-region-properties (point-min) (point-max) '(owner tinydiff))
...
Lets user to edit option in the command line."
  (interactive
   (progn
	...
	(ti::keymap-function-bind-info 'tinydiff-cl-insert-file-autosave
				  tinydiff-:minibuffer-map)
	(sit-for 1)))
     (list
  ...
  (let* ((bf       (buffer-file-name))
    ...
    (cond
     ((setq tmp (ti::string-match "-r\\([0-9.]+\\)" 1 cmd))
      ...
      ;;
      (if (or t
              (null (tinydiff-cl-read-rcs-file-name cmd)))
...
(defun tinydiff-block-region  ()
  ...
  (let* (;; (type   (ti::buffer-diff-type-p))
    ...
    (end-of-line)
    (setq end (point))
    (tinydiff-goto-next nil nil 'no-update)  ;; *** 2720,2727 ****
...
(defun tinydiff-block-kill  ()
  ...
  (let* ((region (tinydiff-block-region))
	 buffer-read-only)
    (if (and (interactive-p) (null region))
...
(defun tinydiff-block-apply-patch  ()
  ...
  (interactive)
  (let* ((region  (save-excursion (tinydiff-block-region)))
	 (header  (tinydiff-header))
	...
	(tinydiff-patch nil nil nil  file 'verb 'hunk)))))
...
(add-hook 'tinydiff-:parse-buffer-hook      'turn-on-font-lock)
