...
;;; @(#) $Id: tinydesk.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tinydesk-version
...
;;
;;	(define-key ctl-x-4-map "s" 'tinydesk-save-state)     ;; free
;;	;;  This is find-file-read-only-other-window, do you ever use it?
;;	(define-key ctl-x-4-map "r" 'tinydesk-recover-state)   ;; Not free
;;	(define-key ctl-x-4-map "e" 'tinydesk-edit-state-file) ;; free
;;	(define-key ctl-x-4-map "u" 'tinydesk-unload)          ;; free
;;
;;   If you have any questions, use this function:
;;
;;      M-x tinydesk-submit-bug-report       send question, feedback,bug report
;;
...
(defcustom tinydesk-:save-before-hook nil
  "*Hook run just before _putting_ files to STATE file.
...
(defcustom tinydesk-:comment-characters ";#"
  "*A string containing all possible comment start characters."
  :type  'string
  :group 'TinyDesk)

(defcustom tinydesk-:mode-name "StateFileEdit"
...
(defcustom tinydesk-:auto-save-interval 5
  "*Interval between doing auto-save of Emacs.
If you set it to 5, after every 5th `write-file' the state is saved.
...
first element and it will be stored to directory `tinydesk-:directory-location'
...
(defcustom tinydesk-:save-title
  ;;  The \$\Docid: is used by tinymy.el
  '(progn
     (format
      (concat
       ";; @(#) Emacs file: tinydesk.el state file\n"
       ";;\n"
       ";;       \$\Docid: %s $\n"
       ";;\n"
...
(defun tinydesk-default-mode-bindings ()
  ...
  ;; Select fast prefix "'" for this key. The C-m is RET key.

  (define-key tinydesk-mode-map "'\C-m" 'tinydesk-load-file)
...
o  add string \"tinydesk-autosave-\"
...
References:
  ...
  `tinydesk-:auto-save-name-function'  function to generate name of state file."
  (interactive "P")
  (let* ((backup-inhibited t)
	 (func       tinydesk-:auto-save-name-function)
	 ...
	 (fn         (concat
		      ...
		      "tinydesk-autosave-" (or name "saved")));; default
	 (save-to    fn))
    ...
    (when (stringp save-to)
      ...
      ;;  time's up? Select name FN if it's string.
	...
	(setq save-to (if (stringp save-to)
			  save-to
			fn))
...
(defun tinydesk-unload-current-file  ()
  ...
  (let* ((file (tinydesk-file-name-absolute
	...
	(message "TinyDesk: No such buffer."))
	  ...
	  (and (move-to-column tinydesk-:message-column 'force)
	       (insert ";;")))
...
directory part, instead of the <N>, so that the names would read:
  ...
  (when (and (string-match "<[0-9]+>$" (buffer-name))
    ...
    (let* ((dir  (file-name-directory    (buffer-file-name)))
	   ...
	   (dir1 (replace-regexps-in-string
		  "/" "-"
		  (or (ti::string-match
		       (concat
			;;  Get Two levels up
...
Input:
  ...
  POP           if non-nil, then pop to first buffer in the saved
...
Return:
  ...
  (let* ((count		0)
	 ...
	 (empty-re      "^[ \t]*$")
	 (msg-str       (if recover "Recovering" "Loading"))
	 first-entry			;
	 bp
	 not-loaded
