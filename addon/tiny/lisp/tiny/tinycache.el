...
;;; @(#) $Id: tinycache.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tinycache-version
...
;;
;;	C-u M-x tinycache-advice-control
;;
...
(defcustom tinycache-:load-hook '(tinycache-install-msb)
...
(defvar tinycache-:mode-user-flag nil
  "Non-nil means that user has touched the `tinycache-mode' flag in this buffer.")
...
;;;  - The view-mode is minor mode in later emacs releases, others
;;;    are encouraged to get mview.el which is minor mode for any emacs.
;;;
;;;            ~/modes/mview.el.Z
;;;            Mike Williams           <mikew@gopher.dosli.govt.nz>
;;;
(defun tinycache-maybe-view-mode ()
  ...
  (let* ((zip-re   ;; arc-mode's zip file prompt
	...
	(ti::funcall 'view-mode 0))	;in 19.28 won't work, but try anyway
     ...
     (t
      (unless (fboundp 'view-mode) (autoload 'view-mode "view" t t))
      (setq func 'view-mode)))

    (save-excursion
      (ti::pmin)
      (if (and (looking-at zip-re)
