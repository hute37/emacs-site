...
;;; @(#) $Id: tinylib.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        extensions
;;
;; To get information on this program use ident(1) or do M-x tinylib-version
...
(defvar ti::-x-coord 170
  "*Default X menu coordinate.")

(defvar ti::-y-coord 170
  "*Default X menu coordinate.")

;; Make this invisible by default, note leading space.
(defvar ti::-passwd-buffer " *passwd-entries*"
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::string-trim-blanks (str)
  "Delete leading and trainling whitespace."
  (when str
    (when (string-match "^[ \t\f\r\n]+\\([^ \t\f\r\n].*\\)" str)
...
;;; ----------------------------------------------------------------------
;;; #todo: Use replace-regexps-in-string
;;;
...
used like that right a way for regexp. Eg in buffer \\\\ two slashes mean
...
(defsubst ti::vc-rcs-normal-file (rcs-file)
  ...
  (let* (( case-fold-search nil))
    (when (ti::vc-rcs-file-p rcs-file)
      (setq rcs-file (replace-regexps-in-string "RCS/" "" rcs-file))
      (setq rcs-file (replace-regexps-in-string ",v"  "" rcs-file)))
    rcs-file))
  ...
  (let* ((re (or re "."))               ;default to match all
    ...
    (cond
     ((ti::string-match-case "RCS/?" d)
      ...
      (dolist (elt list)
        (set fn (replace-regexps-in-string ",v$" "" elt))
        (push fn ret)))
     (t
      (setq list (directory-files d nil re))
...
;;;    (setq BL branch-list VL v-list V version)
...
;;}}}
;;{{{ Version control, buffer's RCS strings
...
Return:
  ...
  ;;
  ;;	    $ Revision: 1.3.2.1 $ / $Date: 2005-12-04 20:58:50 $
...
;;; ----------------------------------------------------------------------
;;; 1995-06-05, mucit@cs.rochester.edu (Bulent Murtezaoglu), comp.emacs
;;;
...
Return:
  ...
  7 tz     time zone	eg, [+-]nnnn, where n = number"
  (let* (wd
	 dd
	 mm
	 m
...
;;; ----------------------------------------------------------------------
;;; #defalias  (defalias 'syntax-info 'ti::string-syntax-info)
;;;
...
Return:
 ...
 nil                    ,see TERMINATE"
   (let* (ret
     ...
     (while level-list
;;;       (ti::d! (car match-list) (car level-list))
...
;;; ----------------------------------------------------------------------
;;; 1994-12-18 <nj104@amtp.cam.ac.uk> (Neil Jerram) gnu.emacs.help
;;; - slightly modified by [jari]
;;;
...
;;; ----------------------------------------------------------------------
;;; - This is handy many times.
;;;
(defun ti::string-1-space (str)
  "Convers all spaces/tabs in STR into 1 space."
  (let* ((out "")
         beg
	 end)
...
Return:
  ...
  (let* (
         (case-fold-search  nil)                ;must use case sensitive
         (beg-re            "begin[ \t]+[0-9]+[ \t]+.")
         (end-re            "end[ \t]*$")
         beg end
         bol
...
;;; ----------------------------------------------------------------------
;;; - Original implementation in c-comment-edit.el/within-c-comment-p
;;;   by kyle_jones@wonderworks.com
;;;
...
(defun ti::buffer-read-line (&optional len skip)
  "Read whole line from buffer. Optionally LEN chars.
If LEN is more than line has characters then Return whole line.
Optionally ignores SKIP count chars at front.
...
;;; Subject:    Re: o-looking-at backwards?
...
;;; ----------------------------------------------------------------------
;;; - originally developed for tinyeat.el
;;;
...
;;; ----------------------------------------------------------------------
;;; This wonderfull basic code was found from
;;; findlib.el  -- Bill Brodie <wbrodie@panix.com>
;;; Slightly rewritten by [jari]
;;;
...
;;;    (ti::d! "point" (point) mode (memq mode '(end nil)))
    ...
    (if (eq 0 count)
        ...
        (if (char-in-list-case (following-char) '(?\t ?\ ))
            (funcall next-skip " \t"))
...
;;; ----------------------------------------------------------------------
;;; 2000-12-20 Colin Walters <walters+n@cis.ohio-state.edu> gnu.emacs.sources
;;;
...
;;; ----------------------------------------------------------------------
;;; Tue May 7, 1996 - feren@ctron.com (Andrew C. Feren), gnu.emacs.sources
...
;;; ----------------------------------------------------------------------
;;; - Original Idea by ericb@col.bsf.alcatel.fr (Eric Blanchard)
;;;   18 Nov 94 in comp.emacs
;;; - slightly rewritten by [jari] <jari.aalto@poboxes.com>
;;;
...
;;; ----------------------------------------------------------------------
;;; Tue, 11 Apr 95 , ralf@prl.philips.nl (Ralf Fassel), comp.emacs
;;;
...
;;;
;;; 28 Jan 1996, ttn@netcom.com (Thien-Thi Nguyen), comp.emacs
;;;
...
;;; ----------------------------------------------------------------------
;;; date unknown, Masanobu UMEDA, umerin@mse.kyutech.ac.jp
;;;
...
;;; 07 Feb 1996, Kai Grossjohann <grossjoh@ls6.informatik.uni-dortmund.de>
;;; Slightly generalized by [jari]
;;;
    ...
    (save-restriction
      ...
      (while (not (eobp))
        (if (null (looking-at empty-line-re))
          ...
          ;;  There is a bug in count-lines, that's why we use line-end-position,
...
;;;       (ti::d! "~count " do-it count nbr pb pe)
          ;; .................................................... action ...
          (cond
           ((null do-it)
            (forward-line 1))           ;skip
...
;;; #defalias (defalias 'trim-blanks 'ti::buffer-trim-blanks)
;;;
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::buffer-xtra-open-outline ()
...
;;; 1995-12-05 gnu.emacs.help
;;;
(defun ti::buffer-count-words (beg end)
  "Count words in region BEG END.
Caveat:
  Region can only start at the beginning of line and end at the end of line."
  (interactive "r")
...
;;; ----------------------------------------------------------------------
;;; - This is quite a handy function when you're programming eg
...
Return:
  nbr   ,count of chars
  nil   ,beg or end delimiter was not found"
  (interactive "P")
  (let* ((alist '(( ?\(  ?\) )
                  ( ?\{  ?\} )
                  ( ?\[  ?\] )
...
;;; ----------------------------------------------------------------------
;;; 1995-10-25 DvdS@eden.com  (David Vanderschel), gnu.emacs.help
;;; - Some such occurrences are valid, so it desn't fix them
;;;
;;; 1995-10-27 mjm@borneo.gmd.de (Matthew Morley), comp.emacs
;;; - Suggested following regexp too:
;;;   (re-search-forward "\\b\\(\\w+\\)\\W+\\1\\b")
;;;
;;; 1995-22-28 kevinr@ihs.com (Kevin Rodgers), gnu.emacs.help
;;; - posted another regexp...
;;;   "\\(\\b\\sw+\\b\\)\\s +\\1"
;;;
;;;
...
(defun ti::window-frame-list  (&optional all exclude-current win)
  "Return only frames that are not dedicated.
Input:
...
;;; ----------------------------------------------------------------------
;;; rpotter@halibut.cis.upenn.edu, (Robert), in rp-describe-function.el
;;;
...
Input:
  ...
  MAP-SYMBOL    map name
  GET-SET       operation 'get 'set 'sett. Where 'set puts the
                functions name behind the PROP only if is there
                is no previous value. 'sett replaces the PROPERTY
                contents.
  PROP          property name
...
Live example:
  ...
  ;;   'my, because The C-cC-c can already be occupied by
  ;;   some other package...
...
;;;
;;; - In general, you should map only keys that are not usually
;;;   "copied" by pasting... such as !; are quite rare..
;;;
;;; - See live translate map example in my guide
;;;   "GUIDE: Emacs key bindings + Xterm and remote emacs -nw"
;;;
(defun ti::keymap-xlat-table (&optional arg)
  "Make new translate table.

Input ARG

  'use      starts using the new table if the
            `keyboard-translate-table' if nil. Otherwise does nothing.
  'use-new  replaces current table with fresh one
  nil       returns new, default translate table.
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::buffer-text-properties-wipe  (beg end)
...
;;; ----------------------------------------------------------------------
;;; 11 Dec 1995, kfogel@floss.cyclic.com (Karl Fogel), gnu.emacs.sources
;;;
;;; - This exellent function by Karl allows user to surround word with
;;;   anything very easily: *emphasize* _unserline_ +smile+ ...
;;;
...
;;; ----------------------------------------------------------------------
;;; 22 Nov 1995, Bruce Montrose <montrose@itd.nrl.navy.mil>, gnu.emacs.help
...
The matches are gathered from `ti::-passwd-buffer' and  if it does not
...
Return:
  ...
  (let* ((passwd-buffer   ti::-passwd-buffer)
...
only once per program. The CMD must be valid Unix command to get
...
References:
  ...
  (let* (;;  Permanent buffer, since launching process is expensive
	 ...
	 (re	    (concat "^" user ":"))
         (buffer    (get-buffer-create ti::-passwd-buffer))
         ret)
    (with-current-buffer buffer
      (unwind-protect
          (progn
...
;;; ----------------------------------------------------------------------
;;; (ti::file-ange-status "/ssjaaa@cs.uta.fi:/")
...
;;; ----------------------------------------------------------------------
;;; - this will find duplicate lisp files in your path.
;;;
;;; 18 Dec 1995, pot@cnuce.cnr.it (Francesco Potorti`)     ,gnu.emacs.help
;;; 19 Dec 1995, Sudish Joseph <joseph@cis.ohio-state.edu> ,gnu.emacs.help
;;; 13 Feb 1996, Jari Aalto [jari], small correction.
;;;
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::file-path-to-unix (path)
  "Convert PATH to Unix / format."
  (replace-char-in-string ?/ ?\\  path))

;;; ----------------------------------------------------------------------
;;;
(defun ti::file-path-to-msdos (path)
  "Convert PATH to MS-DOS \\ format."
  (replace-char-in-string ?\\ ?/  path))

;;; ----------------------------------------------------------------------
;;;
(defun ti::file-make-path  (dir &optional file)
  "Make full path by combining DIR and FILE. NtEmacs compatible.
...
;;; ----------------------------------------------------------------------
;;; Original idea by dcw@gcm.com (David Worenklein)
;;; in gnu.emacs.help Mon, 12 Jun 1995
;;;
...
Return:
  ...
  nil           not unique"
  (let* ((file         (substitute-in-file-name file-name))
    ...
    (setq completed
          (ignore-errors
            ;;   if given impossible entry like "!@#!#" this breaks
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::file-complete-file-name-word  (&optional no-msg)
...
See macro `ti::file-complete-filename-minibuffer' for more.
...
;;;    (ti::d!! word default-directory "\n" )

    (unless (ti::nil-p word)
      (setq word (ti::file-complete-file-name word nil 'list))

;;;      (ti::d!! (null no-msg) (ti::listp word) word "\n" )

      (when (and (null no-msg) (ti::listp word))
        (setq msg
              (format "%d: %s"
                      (length word)
                      (ti::list-to-string word)))
        (message msg)
        (sit-for 0.5))

      (when (ti::listp word)

	;;  Try completion. Returs t if exact match, but
	;;  it shouldn't happen here any more. Still,
	;;  be cautious.

	(cond
	 ((stringp (setq msg (try-completion oword (ti::list-to-assoc-menu word))))
	  (setq word msg))
	 ((stringp (setq msg (try-completion
			      (file-name-nondirectory oword)
			      (ti::list-to-assoc-menu word))))
	  (if (setq dir (file-name-directory oword))
	      (setq word (ti::file-make-path dir msg))
	    (setq word msg)))))
...
;;; ----------------------------------------------------------------------
;;;
(put 'ti::file-complete-filename-minibuffer 'lisp-indent-function 0)
(defmacro ti::file-complete-filename-minibuffer  (&rest body)
  "Complete filename in minibuffer prompt and do BODY.
...
Example 2:
    ...
    (defun my-example (string file-list)
      \"Giving file list as sring while allowing completion on words\"
      (interactive
       (list
  ...
  (`
   (let* ((map (copy-keymap minibuffer-local-map)))
     ;;  this X-event also exists for tab
...
;;;    (ti::d! "fing" connection buffer ret)
...
If COMMAND includes port number, eg:
...
;;;         (ti::d! host port command "sending http....")
	...
	(if connection (delete-process connection))
	;;  Strip Ctrl-M marks
	(with-current-buffer buffer (ti::buffer-lf-to-crlf 'dos2unix))))
...
Return:
  ...
  VERSION   Version number from command line option -version
  TYPE      is 'sun or 'gcc or any orher known Java vendor.
  ...
  (let* ((java  (if binary
	...
	(setq string (buffer-string)))
      ...
      (when (string-match "build[ \t]+\\([0-9_.]+\\)" string)
	(setq version (match-string 1 string)))
...
Return:
  ...
  (let* ((echo-keystrokes 0)            ;prevent showing
         str
	 ch)
    (while (not (char-in-list-case ch '(?\n ?\C-m ?\e)))
      (cond
       ((char-in-list-case ch '(?\b ?\177))
        (if (eq 0 (length str))
            (beep)
...
;;; ..................................................... &bug-reports ...
;;; - I'd advice you to take a look at lisp-mnt.el if you're writing
;;;   your own packages.
...
1. You file must be in RCS control and it must have the \"\$ Id \$\" identifier
...
2. Your Lisp code package must be valid according to lisp-mnt.el's command
...
Input:
  ...
  LIB           filename without path. Eg \"tinylib.el\"
...
;;; ----------------------------------------------------------------------
;;; - So that you don't need to type long function name all the time,
;;;   you could add this into your ~/.emacs
;;;
;;; #defalias  (defalias 'tver 'ti::package-version-info)
;;;
;;;
     ...
     (setq
      ...
      (ti::file-complete-filename-minibuffer
        (read-from-minibuffer
         ...
         nil
	 map)))
     (if (null file) (setq file file))  ;XEmacs 19.14 bytecompiler silencer
...
Input:
  ...
  HEADER-LIST   string or list of strings. Eg '(\"maintainer\")
...
;;;                  (ti::d! (point) (buffer-substring (point) (line-end-position)))
...
otherwise `load-path' is conculted."
  (let* ((xemacs-p (xemacs-p))
	 (ver      (if xemacs-p
		       (emacs-version-number-as-string))) ;eg "19.14"
	 match
	 ret)
    (dolist (path (if xemacs-p
		      load-path
		    exec-path))
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-overlay-properties  (overlay)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-overlay-put (ov-sym prop val)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-overlay-move (ov-sym beg end &optional make-local face)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-activate-region  (&optional off)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-read-password  (&optional prompt)
  ...
  (let* ((var-bind  (boundp 'record-keystrokes))
	...
	(cond
	 ...
	 (t
      ...
      (if record-keystrokes (setq record-keystrokes nil))
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-key-call-original (minor-mode-symbol key-binding)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-mouse-call-original-function (minor-mode-symbol &optional event)
...
See. `ti:-xe-mouse-call-original'."
  (let* (ret)
    ...
    (set minor-mode-symbol nil)
    (setq ret (key-binding (ti::xe-mouse-key event)))
    (set minor-mode-symbol (get minor-mode-symbol 'ti::orig-value))
    ret))


;;; ----------------------------------------------------------------------
;;; This code is originally from folding.el. Function by Anders Lindgren
;;;
(defvar ti:-xe-mouse-call-original nil "See ti::keymap-mouse-call-original.")

(defun ti::xe-mouse-call-original (minor-mode-symbol &optional event)
...
ext-pro  (defun folding-mode-context-sensitive  (event)
    ...
    (ti::xe-mouse-call-original 'folding-mode event))
  ...
  (if ti:-xe-mouse-call-original ;; We're looping already
    ...
    (unwind-protect
        (let* ((orig-buf (current-buffer))
	       (mouse-func (ti::xe-mouse-call-original-function
			    minor-mode-symbol event))
	       (local-func (ti::xe-key-local-map
			    (ti::xe-mouse-key event)))
	       (orig-func  (or local-func
			       mouse-func))
	       (event-p    (when orig-func
			     (string-match
			      "event"
			      (ti::function-args-p orig-func)))))
          (when orig-func
            ;;  Only if existed
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-popup (string-list &optional event mode title)
...
Return:
  ...
  (let* (
         ...
         (event  (or event
                     (ti::xe-make-x-popup-event ti::-x-coord  ti::-y-coord)))
         menu
         item-list
         alist
         ret)
...
;;
(defun ti::xe-make-x-popup-event (x y)
...
Very handy if you call from kbd a function that requires mouse event."
  (cond
   ...
   (t
    ;;; (message "ti::xe-make-x-popup-event, XEmacs implementation not known.")
    nil)))

;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-make-fake-event (x y &optional mouse-sym)
   ...
   ((emacs-p)
    (list
     ...
     (list
      ...
      -23723628)))
   (t
    ;; (message "ti::xe-make-fake-event, XEmacs implementation not known.")
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-set-frame-name (string &optional frame get)
...
If GET is non-nil return frame name."
  (let* ((symbol 'name))
    (when (emacs-p)
      ...
      ;; ..at least my records say so, but I don't know for sure,
      ;; because I don't have 31-33 versions.
      ;;
    ...
    (if get
      ...
      (ti::xe-set-frame-parameter symbol string frame))))

;;; ----------------------------------------------------------------------
;;; Idea by, 23 Mar 1995,  mueller@cs.unc.edu (Carl Mueller), comp.emacs
;;; - Changing the frame label is same as changing the icon label
;;;
;;;
;;;
(defun ti::xe-set-frame-parameter (prop-or-list value &optional frame)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-frame-window-config ()
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-timer-list-control (&optional mode)
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-timer-elt  (function)
...
;;;	(ti::d!! (functionp item) (get-elt elt (1+ pos)))
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-timer-process-status ()
  ...
  (cond
   ...
   ((boundp 'timer-list)                ;Emacs 19.34
    (ti::xe-timer-elt  'display-time-event-handler))
   ((boundp 'itimer-list)               ;
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-timer-cancel  (key &optional cancel-function)
  "Delete timer KEY entry, where KEY is full element in (i)`timer-alist'.
Function `ti::xe-timer-cancel-function' may be more what you want
...
;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-timer-cancel-function (function)
  "Delete all timer entries for FUNCTION."
  (let (key
	ret)
    (while (setq key (car-safe (ti::xe-timer-elt function)))
      (push key ret)
      (ti::xe-timer-cancel key))
    ret))

;;; ----------------------------------------------------------------------
;;;
(defun ti::xe-set-mode-line-format  (fmt)
...
;;;    (push 'progn ret)
    ...
    ;;
    ;; Note 97-09-27
    ;; I started gettings errors from easymenu define command and
    ;; after I forced byte compiler to see the defvar definition
    ;; of the variables during compile time, it started working again.
    ;;
    ;; This was very odd.
    ;;
    ;;  (easy-menu-define
...
FUNC-DEF-SYM KEYMAP-SYM PREFIX-KEYMAP-SYM PREFIX-KEY-SYM
...
;;;		   (symbol-name (quote (` (, debug-variable)) ))
    ...
    ;; Boy this sure is hairy! We are returning a macro  in next
    ;; elt. Watch carefully.
    ;;
    (setq
     elt
...
;;;	   (let* ((ti:m-debug-buffer (, debug-buffer )))
       (list
	'when (quote (, debug-variable))
	(list
	 'let
	 (list
	  (list 'ti:m-debug-buffer (quote (, debug-buffer ))))
;;;	     ;;  We cannot use ,@ in the code directly, because we're
;;;	     ;;  already in backquote, the bq-list* is the equivalent form.
	(list* 'ti::d!! args)
	(list 'ti::d!! "\n")	)))))
