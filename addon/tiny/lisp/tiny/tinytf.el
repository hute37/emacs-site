...
;;; @(#) $Id: tinytf.el,v 1.1 2005-12-04 20:58:50 hute37 Exp $
...
;; Keywords:        wp
;;
;; To get information on this program use ident(1) or do M-x tinytf-version
;; Look at the code with folding.el, tinybm.el

;; LCD Archive Entry:
;; tinytf.el|Jari Aalto|jari.aalto@poboxes.com|
;; TF Technical text format, outline, html markup, text2html conversion|
...
;;
;;* _   (defconst tinytf-:mode-prefix-key "z")   ;; This is the default
;;*     (require 'tinytf)
;;* _
;;
...
;;* _
;;* ;;  Leave out "f" function if you don't want the mode to
;;* ;;  define keys F9-F11 for its own use.
;;* _
...
;;
;;
;; The easiest thing to change the font-lock color settings is to
;; COPY THE WHOLE VARIABLE `tinytf-:font-lock-keywords' from this file
;; to your $HOME/.emacs and make the local changes there. Substitute
;; the `defcustom' with `setq' when you copy the value and delete
;; all :tag values from the variable.
;;
;; You need TF to HTML conversion Perl script available at
;; http://perl-text2html.sourceforge.net/
;;
;;      t2html.pl
;;
;; It is possible to write documentation into other cources in TF format
;; and there is another perl program that is able to extract the documentation
;; into text/plain by omitting comments:
;;
...
(defcustom tinypath-:binary-t2html
...
(defvar tinytf-:idle-timer-elt nil
  "In 19.34+ this hold the idle timer elt activated when mode is turned on.")
(make-variable-buffer-local 'tinytf-:idle-timer-elt)
...
(defcustom tinytf-:idle-time 4
  "*Time in seconds to be idle before column info is displayed."
  :type 'number
  :group 'TinyTf)
...
;;; ----------------------------------------------------------------------
;;; #todo: code by david x callaway <dxc@xprt.net>
;;;
...
(ti::macrof-minor-mode-wizard
 "tinytf-" " Tf" "z" "Tf" 'TinyTf "tinytf-:"              ;1-6
...
"
 ...
 (progn
   (let* ()
     ...
     (define-key map  "z" 'self-insert-command)

     (define-key map  "p" 'tinytf-heading-backward-0)
     (define-key map  "n" 'tinytf-heading-forward-0)
     ...
     ;;  Marking commands somewhere in non-shift symbols
     ;;  I selected  "." to STRONG. Think it like a heavy statement that
     ...
     (define-key map  "-" 'tinytf-mark-word-strong) ;; PERIOTI::D!
     ...
     (define-key map  "\C-p" 'ti::buffer-selective-display-print)
     (define-key map  "\C-c" 'ti::buffer-selective-display-copy-to)

     (define-key map  "\C-m"  'tinytf-column-info-display)
     (define-key map  "x"     'tinytf-exit)
     (define-key map  "X"     'turn-off-tinytf-mode)

     ;;  Original PgUp and down keys --> move under Conrol key
...
(defun tinytf-mode-define-f-keys ()
  "Define default F key to `tinytf-:mode-map'."
  (interactive)
...
buffer to match. The variable includes regexp to match Text headings.
...
(defun tinytf-install (&optional uninstall verb)
  ...
  (add-hook 'write-file-hooks 'tinytf-untabify-buffer uninstall)

  ;;  Always kill old process and start new if install
  ;;  was reqested.

  (ti::xe-timer-cancel-function 'tinytf-idle-command)

  ;;  Install "echo area help"
  ;;  In other emacs releases it is handled in tinytf-untabify

  (unless uninstall
    (if (not (ti::idle-timer-supported-p))
	(message "\
TinyTf: No idle timers. Use \\[tinytf-column-info-display] and \\[untabify].")
      (setq tinytf-:idle-timer-elt
	    (ti::funcall 'run-with-idle-timer
		      tinytf-:idle-time
		      'repeat
		      'tinytf-idle-command))))
  (when verb
  ...
  (let* (case-fold-search
    ...
    (setq
     ...
     (or (ti::re-search-check
	  ...
	  ;;  2) See if you can find two consequtive headings. The
	 ...
	 ;;  3) Alternatively try to find one heading and regular text
	 ;;
	 ;;	    Heading one
	 ;;
...
(defun turn-on-tinytf-mode-maybe ()
  "Turn on mode function `tinytf-mode' if TOC entry is found."
	...
	(cond
	 ...
	 (t
	  (let* ((command (concat
	    ...
	    ;;  Turn on URL recognizer so that you can
	    ;;  click on the printed HTML URL location
...
(defun tinytf-convert-t2html-basic ()
  "Make 1-page HTML using t2html.pl"
  (interactive)
...
(defun tinytf-convert-t2html-frame ()
  ...
  (interactive)
  (let* ((file (tinytf-convert-html-source))
	 (base (concat
		"file:"
...
(defun tinytf-convert-htmlize ()
  ...
  (unless (fboundp 'htmlize-buffer)
    (error "TinyTf: `htmlize-buffer' not available."))
  ...
  (let* ((html-buffer "*html*") ;; not in htmlize.el variable
  ...
  (let* ((sym 'tinytf-mode)
    ...
    (when (or force
      ...
      (let* ((path  (progn
	     ...
	     (path2 (or tinypath-:binary-t2html path))
	     (bin   (and path2
			 (file-exists-p path2)
	...
	(cond
	 (bin
	  ...
	  (put sym 't2html bin))
	 (verb
	  (message "Tinytf: Cannot fint t2html.pl along `exec-path'.")))))
    (when (or force
	...
	(cond
	 (path
	  ...
	  (put sym 'htmlize (locate-library "htmlize.el")))
	 (verb
	  (message "Tinytf: Cannot fin htmlize.pl along `load-path'.")))))
    (list
     (get sym 'htmlize)
     (get sym 't2html))))
...
Note:
  ...
  Update this yourself when you make heading changes.
...
References:
  ...
  (if verb (message "TinyTf: Fixing double spaces..."))
  ...
  (if verb (message "TinyTf: Trimming extra blank lines..."))

  (ti::buffer-trim-blanks (point-min) (point-max)) ;Remove trailing blanks

  (if verb (message "TinyTf: Heading fix..."))

  (tinytf-heading-fix)
  (tinytf-untabify-buffer)
  (tinytf-heading-fix-newlines)

  (if verb (message "TinyTf: Checking heading numbering..."))

  (when (tinytf-headings-numbered-p)
    (tinytf-heading-numbering))

  (if verb (message "TinyTf: Running user fix hooks..."))
...
;;{{{ misc: post/idle functions

;;; ................................................... &post-commands ...

;;; ----------------------------------------------------------------------
;;;
(defun tinytf-idle-command ()
  "Diplay column info when idle.
This function runs only if variable `tinytf-mode' is non-nil."
  (when (and tinytf-mode
             (ti::no-action-in-progress-p 'timer))
    (when (and (boundp 'font-lock-keywords)
	       (or (equal font-lock-keywords nil)
		   (equal font-lock-keywords '(t))))
      ;; Try fixing fontification
      (tinytf-font-lock-mode))
    (tinytf-untabify)
    (tinytf-column-info-display 'suppress)))

;;; ----------------------------------------------------------------------
;;;
(defun tinytf-untabify (&optional force)
  "Untabify buffer in regular intervals. Optionally FORCE untabify.
This function is run only if variable `tinytf-mode' is non-nil."

  (let* ((counter (or (get 'tinytf-untabify 'tinytf-counter)
		      0)))
    (put 'tinytf-untabify 'tinytf-counter (1+ counter))
    (when (or force
              (and tinytf-mode
                   (not buffer-read-only)
                   (> counter 50)))               ;only after X keypresses
      (put 'tinytf-untabify 'tinytf-counter 0)
      (message "TinyTf: Buffer untabified.")
...
(tinytf-fmacro-indent-paragraph
 ...
 10 (format "TinyTf: Quotation, column 10")
 (let* ((fill-prefix (or (ti::string-match "^[ \t]+" 0 (ti::read-current-line)) ""))
        (left-margin 0))
   (fill-paragraph nil)
...
In other places call original function."
  ...
  (cond
   ((or (tinytf-level-p 0)
    ...
    (tinytf-show-toggle))
   (t
    (ti::xe-mouse-call-original 'tinytf-mode event))))
...
(defun tinytf-hide-buffer ()
  ...
  (let* (list
    ...
    (ti::save-line-column-macro nil nil
      (setq list (tinytf-heading-positions))
      (while list
        (setq beg (car list)  end (or (nth 1 list) (point-max)))
        (setq list (cdr list))
...
;;; tinytf.el ends here
; LocalWords:  tinytf defconst tinylib
