...
;;; Modified to work with Windows NT by Steve Kemp
;;; Modified to delete the process buffer when the mode is terminated.
;;;
...
;; $Id: vkill.el,v 1.1 2005-11-26 01:15:01 hute37 Exp $
...
(defvar vkill-show-all-processes t
...
minibuffer."
  ...
  (let ((workbuf (get-buffer-create "*vkill*")))
    ...
    (while (re-search-forward "\\* [0-9a-zA-Z]+[ \t]\\([0-9]+\\).*\n" nil t)
      (replace-match " \\1" t nil))
    (goto-char (point-min))
;
;    (insert "kill -" (if (numberp signal) (int-to-string signal) signal))
;
    (insert kill-program-name)
    (insert " ")
