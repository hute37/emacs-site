...
;;; in `debian-ispell-dictionary-alist' so they are available through
...
(when (and (boundp 'debian-ispell-dictionary-alist)
	   debian-ispell-dictionary-alist)
  (let ((idicts ispell-dictionary-alist))
    ...
    (while idicts
      (let ((dict (car idicts)))
	(unless (assoc (car dict) ispell-dictionary-alist)
	  (setq ispell-dictionary-alist
		(nconc ispell-dictionary-alist (list dict)))))
      (setq idicts (cdr idicts)))))

;;; These defs are already in debian-ispell.el, but since in some moments
;;; both files can be used separately is better to have them here too.
...
(defconst ispell-version (concat ispell-version " (+ Debian patch)"))
...
Otherwise returns the library path if defined."
  ...
  (let ((case-fold-search-val case-fold-search)
	...
	(if (and (= (car (read-from-string (buffer-substring-no-properties
	  ...
	  (message "-- debian-ispell-program-name: %s, ispell-really-aspell %s"
		   debian-ispell-program-name ispell-really-aspell))
...
;; Definition change borrowed from CVS emacs ispell.el
(defvar ispell-library-path (condition-case ()
...
;;; Build pop-up menus for emacs and xemacs

(let ((dicts-full-list (append (mapcar 'car ispell-local-dictionary-alist)
			       debian-ispell-valid-dictionary-list)))
  (if ispell-menu-map-needed
      (let ((dicts (reverse dicts-full-list)))
...
(defun debian-get-coding-system-workaround ()
  (let (ispell-coding-system emacs-coding-system local-buffer-file-coding-system)
    (setq ispell-coding-system
	  (nth 7 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
    ...
    (if (and (boundp 'debian-dict-common-debug)
...
(setq debian-ucs-mule-8859-to-mule-unicode-t
      (if (boundp 'ucs-mule-8859-to-mule-unicode)
	  t
	nil))

(if debian-ucs-mule-8859-to-mule-unicode-t
	...
	(let* ((slot (or
		      (assoc ispell-dictionary ispell-local-dictionary-alist)
		      (assoc ispell-dictionary ispell-dictionary-alist)))
	       (str (nth n slot)))
...
(defun ispell-get-casechars ()
  (if debian-ucs-mule-8859-to-mule-unicode-t
    ...
    (ispell-decode-string
     (nth 1 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
		(assoc ispell-dictionary ispell-dictionary-alist))))))
(defun ispell-get-not-casechars ()
  (if debian-ucs-mule-8859-to-mule-unicode-t
      (ispell-get-decoded-string 2)
    (ispell-decode-string
     (nth 2 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
		(assoc ispell-dictionary ispell-dictionary-alist))))))
(defun ispell-get-otherchars ()
  (if debian-ucs-mule-8859-to-mule-unicode-t
      (ispell-get-decoded-string 3)
    (ispell-decode-string
     (nth 3 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
		(assoc ispell-dictionary ispell-dictionary-alist))))))
(defun ispell-get-many-otherchars-p ()
  (nth 4 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
	     (assoc ispell-dictionary ispell-dictionary-alist))))
(defun ispell-get-ispell-args ()
  (nth 5 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
	     (assoc ispell-dictionary ispell-dictionary-alist))))
(defun ispell-get-extended-character-mode ()
  (nth 6 (or (assoc ispell-dictionary ispell-local-dictionary-alist)
	     (assoc ispell-dictionary ispell-dictionary-alist))))
...
quit          spell session exited."
  ...
  ;; ---- Debian changes
  (if (fboundp 'debian-set-ispell-dictionary) (debian-set-ispell-dictionary))
  ;; ---- End of Debian changes
  (if continue
      (ispell-continue)
    (if (interactive-p)
  ...
  (let ((count ?0)
    ...
    (save-excursion
	  ...
	  (progn
	    ...
	    (insert "\n- Use option `i' if the word is correctly spelled.\n")
...
With NO-ERROR, just return non-nil if there was no Ispell running."
  (interactive)
  ;; Clear flyspell word cache, so current word is rechecked:
  (when (and (featurep 'flyspell)       ; if flyspell is loaded
	     flyspell-mode)             ; and flyspell-mode is active
...
With prefix argument, set the default dictionary."
  (interactive
   (list (completing-read
	  ...
	  (mapcar (function (lambda (dict) (setq dict (cons dict nil))))
...
(if (not (fboundp 'encode-coding-string))
    (defun encode-coding-string (string coding)
      "Dummy function defined in Debian dictionaries-common ispell.el
for use in emacsen flavours (e.g, emacs19, xemacs-nomule) where
real encode-coding-string function is not available. Will return
the string"
      string))
