...
The buffer to mark them in is `flyspell-large-region-buffer'."
  (let (words-not-found
	case-fold-search     ; default nil will make search case-sensitive
	misspell-mismatches
	(ispell-otherchars (ispell-get-otherchars))
	...
	(let ((word (match-string 1)) (wordpos (point)))
	  ...
	  (with-current-buffer flyspell-large-region-buffer
		...
		(if (search-forward word
		    ...
		    (let* ((found-list
		      ...
		      (when (or
			     ;; Strings match, we really found it.
			     (string= word found)
      ...
      ;; we are done
      (if flyspell-issue-message-flag (message "Spell Checking completed.")))
    ;; Warn about not found and unmatched misspellings
    (when debian-debug
      (while words-not-found
	(message "%s: word not found"
		 (car words-not-found))
	(setq words-not-found (cdr words-not-found)))
      (while misspell-mismatches
	(message "%s: misspelled and found string mismatch"
		 (car misspell-mismatches))
	(setq misspell-mismatches (cdr misspell-mismatches))))
    
    ;; Kill and forget the buffer with the list of incorrect words.
    (when (not debian-debug)
...
(defun flyspell-process-localwords (misspellings-buffer)
  (let (localwords
	...
	(while localwords
	  (goto-char (point-min))
	  (let ((regexp (concat "^" (car localwords) "\n")))
...
(defun flyspell-large-region (beg end)
  (let* ((curbuf  (current-buffer))
    ...
    (set-buffer curbuf)
    (ispell-check-version) ;; make sure ispell-really-aspell matches the most current selection
